...
Функция ВыполнитьРассылку(ТаблицаОтчетов, ПараметрыДоставки, Рассылка, ПараметрыЖурнала = Неопределено) Экспорт
	...
	Если ОбщиеВложения.Количество() > 0 Тогда
		...
		//АйдельМС РФП ++	
		//Собираем все вложения в один файл(работает только для .xlsx файлов)	
		Если Рассылка.РФП_ОтправлятьВложенияОднойКнигой Тогда
			ОднаКнига = Новый ПакетОтображаемыхДокументов;
			
			Для Каждого Отчет Из СтрокаПолучатель.Строки Цикл
				Страница = ОднаКнига.Состав.Добавить();
				Страница.Наименование = Отчет.Ключ;
				Страница.Данные = ПоместитьВоВременноеХранилище(Отчет.Значение); 
			КонецЦикла;
			
			ПутьОднойКниги = ПараметрыДоставки.КаталогВременныхФайлов + Рассылка.Наименование + ".xlsx"; 
			
			ОднаКнига.Записать(ПутьОднойКниги, ТипФайлаПакетаОтображаемыхДокументов.XLSX);	
			
			ОбщиеВложения = Новый ТаблицаЗначений;
			ОбщиеВложения.Колонки.Добавить("Ключ", Новый ОписаниеТипов("Строка"));
			ОбщиеВложения.Колонки.Добавить("Значение", Новый ОписаниеТипов("Строка"));
			ОбщиеВложения.Колонки.Добавить("Уровень", Новый ОписаниеТипов("Число"));
			
			НоваяСтрока = ОбщиеВложения.Добавить();
			
			НоваяСтрока.Ключ = Рассылка.Наименование + ".xlsx";
			НоваяСтрока.Значение = ПутьОднойКниги;
