...
//		ПервыйСимвол    = Найти(ШаблонНаименования, "[");
//		ПоследнийСимвол = Найти(ШаблонНаименования, "]");
//		
...
//	|	ВЫРАЗИТЬ(ТоварыДляЗапроса.Номенклатура КАК Справочник.Номенклатура).ВидНоменклатуры КАК ВидНоменклатуры,
//	|	ВЫРАЗИТЬ(ТоварыДляЗапроса.НоменклатураБрак КАК Справочник.Номенклатура).ВидНоменклатуры КАК ВидНоменклатурыБрак
...
//	|	ТоварыДляЗапроса.НомерСтроки,
//	|	ВЫБОР
//	|		КОГДА ТоварыДляЗапроса.ВидНоменклатуры.ИспользоватьСерии
//	|				И НЕ ТоварыДляЗапроса.ВидНоменклатуры.НастройкаИспользованияСерий = ЗНАЧЕНИЕ(Перечисление.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара)
//	|			ТОГДА ТоварыДляЗапроса.Номенклатура
//	|		ИНАЧЕ ТоварыДляЗапроса.НоменклатураБрак
//	|	КОНЕЦ КАК НоменклатураССерией
//	|ИЗ
//	|	ТоварыДляЗапроса КАК ТоварыДляЗапроса
//	|ГДЕ
//	|	ТоварыДляЗапроса.ВидНоменклатуры <> ТоварыДляЗапроса.ВидНоменклатурыБрак
//	|	И (ТоварыДляЗапроса.ВидНоменклатуры.ИспользоватьСерии
//	|				И НЕ ТоварыДляЗапроса.ВидНоменклатуры.НастройкаИспользованияСерий = ЗНАЧЕНИЕ(Перечисление.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара)
//	|			ИЛИ ТоварыДляЗапроса.ВидНоменклатурыБрак.ИспользоватьСерии
//	|				И НЕ ТоварыДляЗапроса.ВидНоменклатурыБрак.НастройкаИспользованияСерий = ЗНАЧЕНИЕ(Перечисление.НастройкиИспользованияСерийНоменклатуры.ЭкземплярТовара))";
//	
...
//	
//	Пока Выборка.Следующий() Цикл
//		ТекстОшибки = НСтр("ru='По номенклатуре %НоменклатураССерией ведется учет по сериям, поэтому вид номенклатуры другого качества (испорченного товара) должен совпадать с видом исходной номенклатуры.'");
//		ТекстОшибки = СтрЗаменить(ТекстОшибки, "%НоменклатураССерией", Выборка.НоменклатураССерией);
...
//				ТипРеквизита = МетаданныеОбъекта.Реквизиты[СтрМас].Тип;
//				ТаблицаТоваров.Колонки.Добавить(СтрМас,Новый ОписаниеТипов(ТипРеквизита));
//			КонецЦикла;
//		КонецЕсли;
...
//		
//		СтруктураПоиска = Новый Структура("Номенклатура, Характеристика, Назначение" + ?(ПараметрыУказанияСерий.ЭтоЗаказ, ", УказыватьСерии", ""));
...
//			Если ПараметрыУказанияСерий.ЭтоЗаказ Тогда
//				Если (НоменклатураКлиентСервер.ВЭтомСтатусеСерииНеУказываются(Выборка.СтатусУказанияСерий)) Тогда
//					Объект.УказыватьСерии = Ложь;
//				Иначе
...
//		ЕстьТоварВШапке = Истина;
//		ПараметрыУказанияСерийПроверка = ПараметрыУказанияСерий.ТЧ;
//		Если ДокументОбъект.СтатусУказанияСерий <> 14 Тогда
//			ДокументОбъект.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
//		КонецЕсли;
//	Иначе
//		ПараметрыУказанияСерийПроверка = ПараметрыУказанияСерий;
//		ЕстьТоварВШапке = ПараметрыУказанияСерийПроверка.ТоварВШапке;
//	КонецЕсли;
//	
//	ЕстьТЧСерии        = ПараметрыУказанияСерийПроверка.ИмяТЧТовары <> ПараметрыУказанияСерийПроверка.ИмяТЧСерии;
//	ЕстьСерииВТЧТовары = Истина;
//	ПерваяСтрока = Истина;
//	
//	Если ПараметрыУказанияСерийПроверка.ИмяТЧТовары <> "" Тогда
//		Для Каждого СтрТабл из ДокументОбъект[ПараметрыУказанияСерийПроверка.ИмяТЧТовары] Цикл
//			
//			Если ПерваяСтрока Тогда 
//				ПерваяСтрока = Ложь;
//				Если Не ОбщегоНазначенияУТКлиентСервер.ЕстьРеквизитОбъекта(СтрТабл, "Серия") Тогда
//					ЕстьСерииВТЧТовары = Ложь;
//					Прервать;
//				КонецЕсли;
//			КонецЕсли;
//			
//			Если ПараметрыУказанияСерийПроверка.ИменаПолейСтатусУказанияСерий.Количество() = 0 Тогда
//				Если Не НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(СтрТабл.СтатусУказанияСерий, ПараметрыУказанияСерийПроверка)
//					Или НоменклатураКлиентСервер.ВЭтомСтатусеСерииНеУказываются(СтрТабл.СтатусУказанияСерий)Тогда
//					
//					СтрТабл.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
//					
//				КонецЕсли;
//			Иначе
//				Для Каждого ИмяПоляСтатус из ПараметрыУказанияСерийПроверка.ИменаПолейСтатусУказанияСерий Цикл
...
//						
//						И (НоменклатураКлиентСервер.ВЭтомСтатусеСерииНеУказываются(СтрТабл[ИмяПоляСтатус])
//						Или Не НоменклатураКлиентСервер.ВЭтомСтатусеСерииУказываютсяВТЧТовары(СтрТабл.СтатусУказанияСерий, ПараметрыУказанияСерийПроверка)) Тогда
//						
...
//	КонецЕсли; 
//	
//	Если Не ЕстьТЧСерии Тогда
//		Возврат;
//	КонецЕсли;
//	
//	Если ПараметрыУказанияСерийПроверка.ИмяТЧТовары <> "" Тогда
//		ТаблицаТовары = ДокументОбъект[ПараметрыУказанияСерийПроверка.ИмяТЧТовары].Выгрузить();
//	Иначе
...
//		ТаблицаТовары.Колонки.Добавить("СтатусУказанияСерий", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(2)));
//	КонецЕсли; 
//	
...
//		
//		Если ПараметрыУказанияСерийПроверка.ИмяТЧТовары = "" Тогда
//			ТаблицаТовары.Колонки.Добавить(СтрМас, Новый ОписаниеТипов(МетаданныеОбъекта.Реквизиты[СтрМас].Тип));
//		КонецЕсли; 
//	КонецЦикла;
//	
//	Запрос = Новый Запрос;
...
//	|	ИЛИ ЕСТЬNULL(ТаблицаТоваров.СтатусУказанияСерий, 0) = 16
//	|	ИЛИ (&ЕстьСерииВТЧТовары 
//	|			И ЕСТЬNULL(ТаблицаТоваров.СтатусУказанияСерий, 0) = 14)
//	|
//	|СГРУППИРОВАТЬ ПО
//	|	ТаблицаСерий.НомерСтроки
//	|
...
//	Запрос.УстановитьПараметр("ЕстьСерииВТЧТовары",ЕстьСерииВТЧТовары);
//	Запрос.УстановитьПараметр("ТаблицаСерий",
//			ДокументОбъект[ПараметрыУказанияСерийПроверка.ИмяТЧСерии].Выгрузить());
//	
//	Выборка = Запрос.Выполнить().Выбрать();
//	
//	Пока Выборка.Следующий() Цикл
...
//	
//	Если ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.Серия, "ВидНоменклатуры") <>
//		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущаяСтрока.Номенклатура, "ВидНоменклатуры") Тогда
//		
...
//	КонецЕсли;
//	
//	Если ТекущаяСтрока.Свойство("УказыватьСерии") Тогда
//		СтруктураВозврата.УказыватьСерии = ТекущаяСтрока.УказыватьСерии;
//	КонецЕсли;
//	
//	Если Не ПараметрыУказанияСерий.ИспользоватьСерииНоменклатуры Тогда
//		
...
//	Иначе
//		Для Каждого ИмяПоляСтатус из ПараметрыУказанияСерий.ИменаПолейСтатусУказанияСерий Цикл
//			ТаблицаТоваров.Колонки.Добавить(ИмяПоляСтатус,Новый ОписаниеТипов("Число",Новый КвалификаторыЧисла(2,0,ДопустимыйЗнак.Неотрицательный)));
//		КонецЦикла;
//	КонецЕсли;
//	
//	Если ПараметрыУказанияСерий.ЭтоЗаказ Тогда
...
//		Для Каждого СтрМас из ПараметрыУказанияСерий.ПоляСвязи Цикл
//			ТипРеквизита = МетаданныеОбъекта.ТабличныеЧасти[ПараметрыУказанияСерий.ИмяТЧТовары].Реквизиты[СтрМас].Тип;
//			ТаблицаТоваров.Колонки.Добавить(СтрМас,Новый ОписаниеТипов(ТипРеквизита));
//			
//			ТипРеквизита = МетаданныеОбъекта.ТабличныеЧасти[ПараметрыУказанияСерий.ИмяТЧСерии].Реквизиты[СтрМас].Тип;
//			ТаблицаСерий.Колонки.Добавить(СтрМас,Новый ОписаниеТипов(ТипРеквизита));
//		КонецЦикла;
//	КонецЕсли;
//	
//	Если ПараметрыУказанияСерий.ИменаПолейДополнительные.Количество() > 0 Тогда

//		Для каждого ИмяДополнительногоПоля из ПараметрыУказанияСерий.ИменаПолейДополнительные Цикл
//			
//			ТипРеквизита = МетаданныеОбъекта.ТабличныеЧасти[ПараметрыУказанияСерий.ИмяТЧТовары].Реквизиты[ИмяДополнительногоПоля].Тип;
//			ТаблицаТоваров.Колонки.Добавить(ИмяДополнительногоПоля, Новый ОписаниеТипов(ТипРеквизита));
//			
//		КонецЦикла;
//		
//	КонецЕсли;
...
//		КонецЕсли;
//		
//		Если НоменклатураКлиентСервер.ВЭтомСтатусеСерииНеУказываются(СтруктураВозврата.СтатусУказанияСерий) Тогда
//			СтруктураВозврата.УказыватьСерии = Ложь;
//			СтруктураВозврата.Серия = Справочники.СерииНоменклатуры.ПустаяСсылка();
//		КонецЕсли;
//		
...
//	
//	
//	УчитыватьУпаковочныеЛисты = ПараметрыУказанияСерий.ПоляСвязи.Найти("УпаковочныйЛистРодитель") <> Неопределено;
//	
...
//	|			ИЛИ ДанныеДокумента.СтатусУказанияСерий = 6)
//	|	И &УсловиеПоУпаковочнымЛистам
//	|;
//	|
//	|////////////////////////////////////////////////////////////////////////////////
//	|ВЫБРАТЬ
...
//	|ИЗ
//	|	ТекстЗапросаОстаткиСерий КАК СерииТоваров
//	|ГДЕ
...
//	|				ИЗ
//	|					ТаблицаТоваров)
//	|	ИЛИ НЕ &УсловиеПоУпаковочнымЛистам
//	|;
//	|
//	|////////////////////////////////////////////////////////////////////////////////
//	|ВЫБРАТЬ
...
//	|";
//	
//	Если УчитыватьУпаковочныеЛисты
//		И Объект.Проведен Тогда
//		// Если часть товаров с сериями успели упаковать в упаковочные листы, для них серии не перезаполняем,
//		//	нужно уменьшить свободный остаток на это количество
//		
//		ТекстЗапросаОстаткиСерий = ТекстЗапросаОстаткиСерий + "
//		|
//		|ОБЪЕДИНИТЬ ВСЕ
//		|
//		|" +
//		"ВЫБРАТЬ
//		|	ТаблицаСерийВУпаковочныхЛистах.Номенклатура,
//		|	ТаблицаСерийВУпаковочныхЛистах.Характеристика,
//		|	ТаблицаСерийВУпаковочныхЛистах.Назначение,
//		|	ТаблицаСерийВУпаковочныхЛистах.Серия,
//		|	&Склад,
//		|	-ТаблицаСерийВУпаковочныхЛистах.Количество КАК СвободныйОстаток
//		|ИЗ
//		|	ТаблицаСерий КАК ТаблицаСерийВУпаковочныхЛистах
//		|ГДЕ
//		|	ТаблицаСерийВУпаковочныхЛистах.УпаковочныйЛистРодитель <> ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)";
//		
//	КонецЕсли;
//	
//	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ТекстЗапросаОстаткиСерий", "(" + ТекстЗапросаОстаткиСерий + ")");
//	
...
//	
//	Если УчитыватьУпаковочныеЛисты Тогда
//		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоУпаковочнымЛистам",
//			"ДанныеДокумента.УпаковочныйЛистРодитель = ЗНАЧЕНИЕ(Документ.УпаковочныйЛист.ПустаяСсылка)");
//	Иначе
//		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&УсловиеПоУпаковочнымЛистам",
//			"ИСТИНА")
//	КонецЕсли;
...
//	
//	Запрос.УстановитьПараметр("ТаблицаТоваров",
//		Объект[ПараметрыУказанияСерий.ИмяТЧТовары].Выгрузить(,КлючевыеПоля + ",СтатусУказанияСерий,НомерСтроки,Количество"));
//	
//	Запрос.УстановитьПараметр("ТаблицаСерий",
//			Объект[ПараметрыУказанияСерий.ИмяТЧСерии].Выгрузить(,КлючевыеПоля + ",Серия,Количество"));
//	
...
//	ОстаткиПоСтрокамТоваров = Новый Соответствие; // Строки товаров могут относиться к разным сериям
...
//	
//	ЕдиницыИзмерения = РезультатЗапроса[6].Выгрузить();
...
//		
//		Если Не СкладыВТЧ Тогда
//			ТекстСообщения = СтрЗаменить(ТекстСообщения,"%ПредставлениеСклада%", СкладыСервер.ПолучитьПредставлениеСклада(ТекущийСклад,Помещение));
//		Иначе
...
//			Или ДокументОбъект.СтатусУказанияСерий = 3
//			Или ДокументОбъект.СтатусУказанияСерий = 5
//			Или ДокументОбъект.СтатусУказанияСерий = 7
//			Или ДокументОбъект.СтатусУказанияСерий = 9 Тогда
//			
//			ТекстСообщения = НСтр("ru = 'Для номенклатуры ""%Товар%"" необходимо указать серии'");
//			
...
//		КонецЕсли;
//		
//		Если ДокументОбъект.СтатусУказанияСерий = 13 Тогда
...
//	Если ПараметрыУказанияСерий.ИмяТЧТовары = ПараметрыУказанияСерий.ИмяТЧСерии Тогда
//		
//		Если ПараметрыУказанияСерий.СкладскиеОперации.Найти(Перечисления.СкладскиеОперации.ОтборИзЯчеек) <> Неопределено
//			Или ПараметрыУказанияСерий.СкладскиеОперации.Найти(Перечисления.СкладскиеОперации.РазмещениеВЯчейки) <> Неопределено Тогда
//			
//			ТекстЗапроса = 
//			"ВЫБРАТЬ
//			|	ТаблицаСерий.НомерСтроки КАК НомерСтроки,
...
//			Для Каждого Суффикс из Суффиксы Цикл
//				ТекстЗапроса = ТекстЗапроса + "
//				|	ТаблицаСерий.Номенклатура" + Суффикс + " КАК Номенклатура" + Суффикс + ",
//				|	ТаблицаСерий.Серия" + Суффикс + "  КАК Серия" + Суффикс + ",
...
//				|	ВЫБОР
//				|		КОГДА ТаблицаСерий.Серия" + Суффикс + " = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
//				|			ТОГДА ИСТИНА
//				|		ИНАЧЕ ЛОЖЬ
...
//			Для Каждого Суффикс из Суффиксы Цикл
//				ТекстЗапроса = ТекстЗапроса + "
//				|	ИЛИ (ТаблицаСерий.СтатусУказанияСерий" + Суффикс + " <> 0
//				|	И (ТаблицаСерий.Серия" + Суффикс + " = ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
...
//						
//						ИмяРеквизитаСерия = МетаданныеДокумента.ТабличныеЧасти[ПараметрыУказанияСерий.ИмяТЧТовары].Реквизиты["Серия" + Суффикс].Синоним;
//						
...
//						
//						Поле = ОбщегоНазначенияКлиентСервер.ПутьКТабличнойЧасти(ПараметрыУказанияСерий.ИмяТЧТовары, Выборка.НомерСтроки, "Серия" + Суффикс);
//						
//						ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,КлючДанных,Поле,"Объект",Отказ);
//					КонецЕсли;
//					
...
//	|	ТаблицаТоваров.Номенклатура,
//	|	ТаблицаТоваров.Характеристика,
//	|	ТаблицаТоваров." + ПараметрыУказанияСерий.ИмяПоляКоличество + " КАК Количество,
//	|	ТаблицаТоваров.СтатусУказанияСерий,
//	|	ТаблицаТоваров.НомерСтроки
//	|ПОМЕСТИТЬ ТаблицаТоваров
//	|ИЗ
//	|	&ТаблицаТоваров КАК ТаблицаТоваров
//	|ГДЕ
//	|   ТаблицаТоваров.СтатусУказанияСерий В (1,3,5,7,9,13)
//	|;
//	|////////////////////////////////////////////////////////////////////////////////
//	|ВЫБРАТЬ " +
//	ТекстПоляВыбораТовары + "
...
//	|	ТаблицаСерий.Номенклатура,
//	|	ТаблицаСерий.Характеристика,
//	|	ТаблицаСерий." + ПараметрыУказанияСерий.ИмяПоляКоличество + " КАК Количество
//	|ПОМЕСТИТЬ ТаблицаСерий
//	|ИЗ
//	|	&ТаблицаСерий КАК ТаблицаСерий
//	|;
...
//// 		* КлючУникальности - Число - имеет смысл для формы подбора серий. В заказах строки товаров, для которых серии указаны, группируются, по ним количество товаров суммируется. Строки, в которых
////								серии не указаны, не группируются и в форме подбора серий такие строки, даже если в них один и тот же товар, при пролистывании
////								показываются отдельно. Ключ уникальности для строк, для которых серии НЕ указаны - это номер строки, для строк, для которых
////								серии указаны - 0. Для товара в шапке заказа на сборку КлючУникальности - всегда 1
//// 		* Склад - СправочникСсылка.Склады
//// 		* Помещение - СправочникСсылка.СкладскиеПомещения
//// 		* УпаковкаДляПодстановки - СправочникСсылка.УпаковкиНоменклатуры, Неопределено - упаковка, в которой нужно выводить количество в форме регистрации серий
...
//	
//	Если ПараметрыУказанияСерий.РегистрироватьСерии Тогда
//		НомераСтрокДокумента = "";
//		
//		Если ЕстьУпаковки
//			И ЗначениеЗаполнено(ТекущиеДанные.Упаковка) Тогда
//			КоэффициентУпаковки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущиеДанные.Упаковка, "Коэффициент");
//		Иначе
...
//						УпаковкаДляПодстановки = Справочники.УпаковкиНоменклатуры.ПустаяСсылка();
//					КонецЕсли;	
//					
//				КонецЕсли;
//				
...
//		
//		Если ПараметрыУказанияСерий.ЭтоЗаказ Тогда
//			ТаблицаТоваров.Колонки.Добавить("КлючУникальности",Новый ОписаниеТипов("Число",
...
//			|	" + ?(НЕ ЕстьУпаковки, "ТаблицаТоваров.Упаковка,", "") + "
//			|	ТаблицаТоваров.НомерСтроки
//			|ПОМЕСТИТЬ ТаблицаТоваров
//			|ИЗ
//			|	&ТаблицаТоваров КАК ТаблицаТоваров
...
//			
//			Если ТЧ.Колонки.Найти("Серия") = Неопределено Тогда
...
//	ПараметрыФормыУказанияСерий.Вставить("РегистрироватьСерии",ПараметрыУказанияСерий.РегистрироватьСерии);
...
//Процедура ОбработатьУказаниеСерий(Объект, ПараметрыУказанияСерий,ПараметрыФормыУказанияСерий, СтруктураДействий = Неопределено, КешированныеЗначения = Неопределено) Экспорт
//	
//	ЕстьУпаковки = ПараметрыУказанияСерий.ПоляСвязи.Найти("Упаковка") <> Неопределено;
//	
...
//							/ ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПараметрыФормыУказанияСерий.Упаковка, "Коэффициент");
//	Иначе
...
//			ИначеЕсли СтруктураДействий.Свойство("ПересчитатьКоличествоЕдиницСуффикс") Тогда
//				КоличествоЕдиницСуффикс = СтруктураДействий.ПересчитатьКоличествоЕдиницСуффикс;
//				СтруктураДействий.Удалить("ПересчитатьКоличествоЕдиницСуффикс");
//				ЕстьПересчетКоличества = Истина;
//			КонецЕсли;
//			
...
//							СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиницСуффикс", КоличествоЕдиницСуффикс);
//						Иначе
//							СтруктураДействий.Вставить("ПересчитатьКоличествоЕдиниц");
//						КонецЕсли; 
//					Иначе
//						Если КоличествоЕдиницСуффикс <> "" Тогда
//							СтруктураДействий.Вставить("ПересчитатьКоличествоУпаковокСуффикс", КоличествоЕдиницСуффикс);
//						Иначе
//							СтруктураДействий.Вставить("ПересчитатьКоличествоУпаковок");
//						КонецЕсли;
//					КонецЕсли;
...
//				
//				ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТоваров, СтруктураДействий, КешированныеЗначения);
//				
...
//						СтрокаТары.КоличествоУпаковок = СтрокаТары.Количество;
//						
//						ОбработкаТабличнойЧастиСервер.ОбработатьСтрокуТЧ(СтрокаТары, СтруктураДействий, КешированныеЗначения);
...
//		
//		СерииУказаныПолностью = (КоличествоСерий = КоличествоТоваров
//								И (КоличествоТоваров > 0 Или ПараметрыУказанияСерий.ЧетныйСтатусДляНулевогоКоличества));
//		
//		Для Каждого СтрМас из НайденныеСтрокиТоваров Цикл
//			
...
//	КонецЕсли;
//	
//	НазначенияВТЧ = ПараметрыУказанияСерийЗаполнение.ПоляСвязи.Найти("Назначение") <> Неопределено;
//	ИспользоватьНазначениеОтправителя = ПараметрыУказанияСерийЗаполнение.ПоляСвязи.Найти("НазначениеОтправителя") <> Неопределено;
//	
//	СкладыВТЧ = ПараметрыУказанияСерийЗаполнение.ПоляСвязи.Найти(ПараметрыУказанияСерийЗаполнение.ИмяПоляСклад) <> Неопределено;
//	
//	ТекстЗапроса = 
//	"ВЫБРАТЬ
//	|	ТаблицаТовары.Номенклатура,
//	|	ТаблицаТовары.Характеристика,
//	|	" + ?(НазначенияВТЧ,"ТаблицаТовары.Назначение","ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)") + " " + "КАК Назначение,
//	|	" + ?(ИспользоватьНазначениеОтправителя,"ТаблицаТовары.НазначениеОтправителя","ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)") + " " + "КАК НазначениеОтправителя,
//	|	ТаблицаТовары.Серия,
...
//	|	РезервыСерийТоваров.Назначение,
...
//	|		РезервыСерийТоваровОстатки.Назначение КАК Назначение,
...
//	|				,
//	|				(ДокументРезерва, Склад, Номенклатура, Характеристика, Назначение) В
//	|						(ВЫБРАТЬ
//	|							ТаблицаТовары.ДокументРезерваСерий КАК ДокументРезерва,
//	|							ТаблицаТовары.Склад,
//	|							ТаблицаТовары.Номенклатура,
//	|							ТаблицаТовары.Характеристика,
//	|							" + ?(ИспользоватьНазначениеОтправителя,"ТаблицаТовары.НазначениеОтправителя","ТаблицаТовары.Назначение") +"
//	|						ИЗ
//	|							ТаблицаТоварыДляЗапроса КАК ТаблицаТовары)
...
//	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаТоварыДляЗапроса КАК ТаблицаТовары
//	|			ПО РезервыСерийТоваров.Номенклатура = ТаблицаТовары.Номенклатура
//	|				И РезервыСерийТоваров.Характеристика = ТаблицаТовары.Характеристика
//	|				И РезервыСерийТоваров.Назначение = " + ?(ИспользоватьНазначениеОтправителя,"ТаблицаТовары.НазначениеОтправителя","ТаблицаТовары.Назначение") +"
//	|				И РезервыСерийТоваров.Склад = ТаблицаТовары.Склад
//	|				И РезервыСерийТоваров.ДокументРезерва = ТаблицаТовары.ДокументРезерваСерий
//	|	ГДЕ
...
//	|	" + ?(ИспользоватьНазначениеОтправителя,"ДанныеРегистра.Назначение","ЗНАЧЕНИЕ(Справочник.Назначения.ПустаяСсылка)") + " " + "КАК НазначениеОтправителя,
//	|	ТаблицаТовары.Назначение,
//	|	ДанныеРегистра.ДокументРезерваСерий,
...
//	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаТоварыДляЗапроса КАК ТаблицаТовары
//	|		ПО ДанныеРегистра.Номенклатура = ТаблицаТовары.Номенклатура
//	|			И ДанныеРегистра.Характеристика = ТаблицаТовары.Характеристика
//	|			И ДанныеРегистра.Назначение = " + ?(ИспользоватьНазначениеОтправителя,"ТаблицаТовары.НазначениеОтправителя","ТаблицаТовары.Назначение") +"
//	|			И ДанныеРегистра.ДокументРезерваСерий = ТаблицаТовары.ДокументРезерваСерий
//	|			И ДанныеРегистра.Склад = ТаблицаТовары.Склад	
//	|;
//	|
//	|////////////////////////////////////////////////////////////////////////////////
//	|ВЫБРАТЬ
...
//	|	РезультирующаяТаблица.НазначениеОтправителя,
//	|	РезультирующаяТаблица.Назначение,
//	|	РезультирующаяТаблица.ДокументРезерваСерий,
//	|	РезультирующаяТаблица.Склад,
//	|	РезультирующаяТаблица.Серия,
//	|	РезультирующаяТаблица.Количество
//	|ИЗ
//	|	РезультирующаяТаблица КАК РезультирующаяТаблица
//	|		ЛЕВОЕ СОЕДИНЕНИЕ ТаблицаТоварыДляЗапроса КАК ТаблицаТовары
//	|		ПО РезультирующаяТаблица.Номенклатура = ТаблицаТовары.Номенклатура
//	|			И РезультирующаяТаблица.Характеристика = ТаблицаТовары.Характеристика
//	|			И РезультирующаяТаблица.Назначение = ТаблицаТовары.Назначение
//	|			И РезультирующаяТаблица.НазначениеОтправителя = ТаблицаТовары.НазначениеОтправителя
//	|			И РезультирующаяТаблица.ДокументРезерваСерий = ТаблицаТовары.ДокументРезерваСерий
//	|			И РезультирующаяТаблица.Склад = ТаблицаТовары.Склад
//	|			И РезультирующаяТаблица.Серия = ТаблицаТовары.Серия
//	|ГДЕ
//	|	ТаблицаТовары.Серия ЕСТЬ NULL";

//	Запрос = Новый Запрос;
//	Запрос.Текст = ТекстЗапроса;
//	
//	Запрос.УстановитьПараметр("Регистратор",Объект.Ссылка);
//	ПоляГруппировки = "Номенклатура,Характеристика,ДокументРезерваСерий,Серия";
//	Если НазначенияВТЧ Тогда
//		ПоляГруппировки = ПоляГруппировки + ",Назначение";
//	КонецЕсли;
//	Если ИспользоватьНазначениеОтправителя Тогда
//		ПоляГруппировки = ПоляГруппировки + ",НазначениеОтправителя";
//	КонецЕсли;
//	Если СкладыВТЧ Тогда
//		ПоляГруппировки = ПоляГруппировки + ",Склад";
//		Запрос.УстановитьПараметр("ТаблицаТовары",
//			Объект[ПараметрыУказанияСерийЗаполнение.ИмяТЧТовары].Выгрузить(,ПоляГруппировки));
//	Иначе
//		ТаблицаТовары = Объект[ПараметрыУказанияСерийЗаполнение.ИмяТЧТовары].Выгрузить(,ПоляГруппировки);
//		
//		Если ЕстьТоварВШапке Тогда
...
//			Возврат;
//		КонецЕсли;												
//	ИначеЕсли Не ПолучитьФункциональнуюОпцию("УчитыватьСебестоимостьПоСериямСклад") 
//		Тогда
//		Возврат;
//	КонецЕсли;
//														
...
//	СписокЗначений.Добавить(12);
//	СписокЗначений.Добавить(16);
...
//	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ЦветТекста", ЦветаСтиля.ТекстЗапрещеннойЯчейкиЦвет);
//	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = '<для работ>'"));
//	ЭлементУсловногоОформления.Оформление.УстановитьЗначениеПараметра("ТолькоПросмотр", Истина);
...
//		Если ПараметрыУказанияСерий.Свойство("ЗаказНаСогласовании") Тогда
//			Запрос.УстановитьПараметр("ЗаказНаСогласовании", ПараметрыУказанияСерий.ЗаказНаСогласовании);
//		КонецЕсли;
//	КонецЕсли; 
...
//	КонецЕсли;
//	
//	Если ПараметрыУказанияСерий.СкладскиеОперации.Найти(Перечисления.СкладскиеОперации.ПриемкаПоПеремещению) <> Неопределено
//		ИЛИ ПараметрыУказанияСерий.СкладскиеОперации.Найти(Перечисления.СкладскиеОперации.ПередачаВПроизводствоПриемка) <> Неопределено Тогда
//		Запрос.УстановитьПараметр("Приемка", Истина);
//	Иначе		
//		Запрос.УстановитьПараметр("Приемка", Ложь);
//	КонецЕсли; 
//	
...
//	
//	Запрос.УстановитьПараметр("ЧетныйСтатусДляНулевогоКоличества", ПараметрыУказанияСерий.ЧетныйСтатусДляНулевогоКоличества);
//	
...
// Возвращает номер серии число(6,0) по ссылке на справочник "СерийныйеНомера"
//
Функция НомерСерии(СерийныйНомер) Экспорт
	
	Номер = 0;
	
	Наименование = Прав(СокрЛП(СерийныйНомер.Наименование), 6);
	
	Попытка
		Номер = Число(Наименование);	
	Исключение
		Возврат Номер;
		//ОписаниеОшибки()
	КонецПопытки; 
...
// Возвращает период серии "Дата" по ссылке на справочник "СерийныйеНомера"
//
Функция ПериодСерии(СерийныйНомер) Экспорт 
	
	ХвостГода = Сред(СерийныйНомер.Наименование, 4, 2);
	
	ГодЧислом = Число("20"+ХвостГода);
	
	Возврат Дата(ГодЧислом, 1, 1);
	
КонецФункции

Функция НаименованиеСоответствуетГодовомуШаблону(СерийныйНомер) Экспорт
	
	НаименованиеСН = СерийныйНомер.Наименование;
	
	Возврат СоответствиеСтрокиМаске("XX-99-999999", НаименованиеСН);
	
КонецФункции
 
Функция СоответствиеСтрокиМаске(Маска, Строка)
	
	Отказ = Ложь;
	
	Символы_Лат = "qwertyuiopasdfghjklzxcvbnmёйцукенгшщзхъфывапролджэячсмитьбю";
	Символы_Лат_Б = "QWERTYUIOPASDFGHJKLZXCVBNMЁЙЦУКЕНГШЩЗХЪФЫВАПРОЛДЖЭЯЧСМИТЬБЮ";
	Символы_Спец = "-)(\/{}?+ ";
	Символы_Циф = "1234567890";
	ДлинаМаски = СтрДлина(Маска);
	ДлинаСтроки = СтрДлина(Строка);
	Сч=0;
	Пока Сч < ДлинаМаски Цикл
		Сч = Сч + 1;
		ТипСимвола = Сред(Маска,Сч,1);
		Если ТипСимвола = "x" И Найти(Символы_Лат,Сред(Строка,Сч,1)) = 0 Тогда
			Отказ = Истина;
		ИначеЕсли ТипСимвола = "X" И Найти(Символы_Лат_Б,Сред(Строка,Сч,1)) = 0 Тогда
			Отказ = Истина;
		ИначеЕсли ТипСимвола = "9" И Найти(Символы_Циф,Сред(Строка,Сч,1)) = 0 Тогда
			Отказ = Истина;
		ИначеЕсли Найти(Символы_Спец,Сред(Строка,Сч,1)) = 0	И ТипСимвола <> "X" И ТипСимвола <> "x" И ТипСимвола <> "9" Тогда
			Отказ = Истина;
		ИначеЕсли ДлинаСтроки <> ДлинаМаски Тогда
			Отказ = Истина;
		КонецЕсли;
	КонецЦикла;
