...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// УНФ
	// заполнение реквизита формы ПрофильРабочееМестоКассира
	Запрос = Новый Запрос(
	"ВЫБРАТЬ
	...
	|ГДЕ
	|	ПрофилиГруппДоступа.ИдентификаторПоставляемыхДанных = &ГУИДПрофиляРМК");
	...
	Если Выборка.Следующий() Тогда
		ПрофильРабочееМестоКассира = Выборка.Ссылка;
	Иначе
		ВызватьИсключение(НСтр("ru='Не найден профиль ""Рабочее место кассира""'"));
	КонецЕсли;
	// Конец УНФ
	...
	Если ТекущиеДанные <> Неопределено
		...
		Если ТекущиеДанные.Профиль = ПрофильРабочееМестоКассира Тогда
			...
			Если СтрокиСПометками.Количество() > 1 Тогда
				...
				ПоказатьВопрос(
					Новый ОписаниеОповещения("ОкончаниеУстановкиПометкиРМК",ЭтотОбъект,СтрокиСПометками),
					НСтр("ru='Для профиля рабочего места кассира выбор других профилей недоступен.
							|Очистить другие профили?'"),
			...
			Если СтрокаРМК.Пометка Тогда
				...
				ПоказатьВопрос(
					Новый ОписаниеОповещения(
						"ОкончаниеОчисткиПометкиРМК",ЭтотОбъект,
						Новый Структура("СтрокаРМК, СтрокаДанных", СтрокаРМК, ТекущиеДанные)),
					НСтр("ru='Профиль рабочего места кассира не совместим с другими профилями.
						|Снять пометку с профиля рабочего места кассира?'"),
					РежимДиалогаВопрос.ДаНет,
					,КодВозвратаДиалога.Да);
...
&НаКлиенте
Процедура ОкончаниеУстановкиПометкиРМК(Результат, СтрокиСПометками) Экспорт
	// УНФ
	
	// Если пользователь согласился с очисткой других профилей
	// тогда вырубаем у него другие профили
	Если Результат = КодВозвратаДиалога.Да Тогда
		Для Каждого Строка Из СтрокиСПометками Цикл
			Если строка.Профиль <> ПрофильРабочееМестоКассира Тогда
				Строка.Пометка = Ложь;
			КонецЕсли;
		КонецЦикла;
	// если пользователь отказался, то вырубаем профиль РМК. 
	Иначе
		Для Каждого Строка Из СтрокиСПометками Цикл
			Если строка.Профиль = ПрофильРабочееМестоКассира Тогда
				Строка.Пометка = Ложь;
...
&НаКлиенте
Процедура ОкончаниеОчисткиПометкиРМК(Результат, СтруктураПеременных) Экспорт
	// УНФ
	
	Если Результат = КодВозвратаДиалога.Да Тогда
		СтруктураПеременных.СтрокаРМК.Пометка = Ложь;
	Иначе
		СтруктураПеременных.СтрокаДанных.Пометка = Ложь;
	КонецЕсли;
