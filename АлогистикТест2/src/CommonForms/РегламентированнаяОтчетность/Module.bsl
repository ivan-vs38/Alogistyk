...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СохраненныеНастройкиСортировки = ХранилищеНастроекДанныхФорм.Загрузить(ЭтаФорма.ИмяФормы, "Форма1СОтчетность_Порядок");
	Если ТипЗнч(СохраненныеНастройкиСортировки) = Тип("ПорядокКомпоновкиДанных") Тогда
		ВостановитьНастройкиСортировки(ЭтаФорма, СохраненныеНастройкиСортировки);
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Процедура ВостановитьНастройкиСортировки(Форма, НастройкиСортировки)
	
	ТекущиеНастройкиСортировки = Новый Массив;
	
	ПользовательскиеНастройкиДинамическогоСписка = Форма.Отчеты.КомпоновщикНастроек.ПользовательскиеНастройки;
	
	Для Каждого ЭлементНастроек Из ПользовательскиеНастройкиДинамическогоСписка.Элементы Цикл
		Если ТипЗнч(ЭлементНастроек) = Тип("ПорядокКомпоновкиДанных") Тогда
			ТекущиеНастройкиСортировки.Добавить(ЭлементНастроек);
			Прервать;
		КонецЕсли;
	КонецЦикла;
	
	Если НЕ ЗначениеЗаполнено(ТекущиеНастройкиСортировки) Тогда
		Возврат;
	КонецЕсли;
	
	ОбновленныеНастройкиСортировки = ТекущиеНастройкиСортировки[0];
	ОбновленныеНастройкиСортировки.Элементы.Очистить();
	
	Для Каждого ЭлементНастроекСортировки Из НастройкиСортировки.Элементы Цикл
		НовыйЭлементНастроекСортировки = ОбновленныеНастройкиСортировки.Элементы.Добавить(Тип("ЭлементПорядкаКомпоновкиДанных"));
		ЗаполнитьЗначенияСвойств(НовыйЭлементНастроекСортировки, ЭлементНастроекСортировки);
	КонецЦикла;
	
	ОбновленныеНастройкиСортировки.ИдентификаторПользовательскойНастройки = НастройкиСортировки.ИдентификаторПользовательскойНастройки;
...
Процедура СохранитьНастройкиОтбораИСортировки()
	...
	Для Каждого ЭлементНастроек Из ПользовательскиеНастройкиДинамическогоСписка.Элементы Цикл
		Если ТипЗнч(ЭлементНастроек) = Тип("ПорядокКомпоновкиДанных") Тогда
			ХранилищеНастроекДанныхФорм.Сохранить(ЭтаФорма.ИмяФормы, "Форма1СОтчетность_Порядок", ЭлементНастроек);
			Прервать;
		КонецЕсли;
	КонецЦикла;
