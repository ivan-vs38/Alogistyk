...
Процедура СформироватьТабличныеДокументыНаСервере(ВыбранныеКоманды)
	...
	Для Каждого ВыбраннаяКоманда Из ВыбранныеКоманды Цикл
		...
		Если Результат.КоллекцияПечатныхФорм.Количество() > 0 Тогда
			...
			Если Результат.ПараметрыВывода.Свойство("Вложения") И ТипЗнч(Результат.ПараметрыВывода.Вложения) = Тип("Соответствие") Тогда
				Для каждого Эл Из Результат.ПараметрыВывода.Вложения Цикл
					Вложения.Добавить(Эл.Ключ, Эл.Значение)
				КонецЦикла;
			КонецЕсли;
...
Функция ПоместитьТабличныеДокументыВоВременноеХранилище()
	...
	Для каждого Вложение Из Вложения Цикл
		ОписаниеФайла = Новый Структура;
		ОписаниеФайла.Вставить("Представление", Вложение.Представление);
		ОписаниеФайла.Вставить("АдресВоВременномХранилище", Вложение.Значение);
		Результат.Добавить(ОписаниеФайла);
	КонецЦикла;
