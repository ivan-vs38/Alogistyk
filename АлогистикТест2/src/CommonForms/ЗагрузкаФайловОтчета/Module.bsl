			...
			Если ОсновнойФайл <> Неопределено Тогда
				...
				Если ЭтоВыгрузкаВXML Тогда
					Если ТипПолучателя = ПредопределенноеЗначение("Перечисление.ТипыКонтролирующихОрганов.ФТС") Тогда
						ПредставлениеТекст = РегламентированнаяОтчетностьЗагрузка.ПрочитатьТекстИзФайла(ФайлОсновногоПредставления, "UTF-8");
					Иначе
						ПредставлениеТекст = РегламентированнаяОтчетностьЗагрузка.ПрочитатьТекстИзФайла(ФайлОсновногоПредставления, , Истина);
					КонецЕсли;
				Иначе
...
Процедура ФайлыКомплектаПередНачаломДобавления(Отказ, Копирование, ЭтоДополнительныеФайлы)
	...
	РегламентированнаяОтчетностьЗагрузкаКлиент.ПолучениеФайловДляЗагрузкиНачало(УникальныйИдентификатор, , ДополнительныеПараметры);
...
Функция СвойстваВидаОтчетаПоИсточнику(Источник)
	...
	Если ИсточникОтчета = "РегламентированныйОтчетБухОтчетность" Тогда // источник отчета из файла ЭП
	...
	ИначеЕсли НайденныйВидОтчета.ТипДокумента = Перечисления.ТипыОтправляемыхДокументов.ПрочаяОтчетностьФНС Тогда
		...
		СвойстваОсновногоФайла = РегламентированнаяОтчетностьЗагрузка.СвойстваФайлаЗагрузки(ПредставлениеТекст, 10, Истина, , Истина);
		
		ВерсияФормата = СвойстваОсновногоФайла["ВерсФорм"];
