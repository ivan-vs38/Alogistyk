...
Процедура ЗаполнитьТаблицуИСоздатьЭлементы()
	...
	Если НесколькоВидовЗаказов Тогда
		...
		Если НЕ ВОднуКолонку Тогда
			Для ии = 1 По КоличествоИтераций Цикл
				...
				Иначе
					Элемент = Элементы.Добавить("Заголовок"+ИмяСостояния, Тип("ДекорацияФормы"), ПодГруппа);
		...
		Пока Выборка.Следующий() Цикл
			...
			Для ии = 1 По КоличествоИтераций Цикл
				Если НесколькоВидовЗаказов И ии=КоличествоИтераций Тогда
					ПодГруппа = Элементы.Добавить("ГруппаСостояние"+Формат(Индекс, "ЧН=0; ЧГ=0"), Тип("ГруппаФормы"), Элементы["Колонка"+ии]);
				...
				Иначе
					ПодГруппа = Элементы["Колонка"+ии];
				КонецЕсли; 
				Элемент = Элементы.Добавить("Состояние"+Формат(Индекс, "ЧН=0; ЧГ=0")+"_"+ии, Тип("ПолеФормы"), ПодГруппа);
				Элемент.ПутьКДанным = "ТаблицаСостояний["+Индекс+"]."+?(ВОднуКолонку, ИмяРеквизита, ИмяРеквизитаПоИндексу(ии));
				...
				КонецЦикла;
				Если НесколькоВидовЗаказов И ии=КоличествоИтераций Тогда
					Элемент = Элементы.Добавить("Отключение"+Формат(Индекс, "ЧН=0; ЧГ=0"), Тип("ПолеФормы"), ПодГруппа);
					Элемент.ПутьКДанным = "ТаблицаСостояний["+Индекс+"].НеИспользоватьДоставку";
	...
	Иначе
		...
		Пока Выборка.Следующий() Цикл
			...
			Индекс = ТаблицаСостояний.Индекс(СтрокаТабличнойЧасти);
			Для ии = 1 По КоличествоИтераций Цикл
				Элемент = Элементы.Добавить("Состояние"+Формат(Индекс, "ЧН=0; ЧГ=0")+"_"+ии, Тип("ПолеФормы"), Элементы.Колонка1);
				Элемент.ПутьКДанным = "ТаблицаСостояний["+Индекс+"]."+?(ВОднуКолонку, ИмяРеквизита, ИмяРеквизитаПоИндексу(ии));
				Элемент.Вид = ВидПоляФормы.ПолеВвода;
				Элемент.КнопкаОткрытия = Ложь;
