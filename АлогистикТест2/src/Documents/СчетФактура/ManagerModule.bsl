...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылкаСчетФактура, СтруктураДополнительныеСвойства) Экспорт
	...
	Если ЭтоПродажа Тогда
		...
		СтрокаОснованиеДоставки.ДокументОснование = ДокументСсылкаСчетФактура.ДокументОснованиеДоставки;
	...
	Запрос.УстановитьПараметр("ОтражатьДоставку", ЗначениеЗаполнено(ДокументСсылкаСчетФактура.НоменклатураДоставки));
	...
	|	СчетФактура.СтавкаНДСДоставки,
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ЛОЖЬ
	|				ТОГДА 0
	|			ИНАЧЕ ВЫБОР
	|					КОГДА СчетФактура.ВалютаДокумента = &ВалютаНациональная
	|						ТОГДА СчетФактура.СуммаНДСДоставки * ВТ_КурсыДокОсн.КурсРегВал * ВТ_КурсыДокОсн.КратностьУпрВал / (ВТ_КурсыДокОсн.КурсУпрВал * ВТ_КурсыДокОсн.КратностьРегВал)
	|					ИНАЧЕ СчетФактура.СуммаНДСДоставки * ВТ_КурсыДокОсн.Курс * ВТ_КурсыДокОсн.КратностьУпрВал / (ВТ_КурсыДокОсн.КурсУпрВал * ВТ_КурсыДокОсн.Кратность)
	|				КОНЕЦ
	...
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА СчетФактура.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА (СчетФактура.СтоимостьДоставкиБезНДС + СчетФактура.СуммаНДСДоставки) * ВТ_КурсыДокОсн.КурсРегВал * ВТ_КурсыДокОсн.КратностьУпрВал / (ВТ_КурсыДокОсн.КурсУпрВал * ВТ_КурсыДокОсн.КратностьРегВал)
	|			ИНАЧЕ (СчетФактура.СтоимостьДоставкиБезНДС + СчетФактура.СуммаНДСДоставки) * ВТ_КурсыДокОсн.Курс * ВТ_КурсыДокОсн.КратностьУпрВал / (ВТ_КурсыДокОсн.КурсУпрВал * ВТ_КурсыДокОсн.Кратность)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)),
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА ЛОЖЬ
	|				ТОГДА 0
	|			ИНАЧЕ ВЫБОР
	|					КОГДА СчетФактура.ВалютаДокумента = &ВалютаНациональная
	|						ТОГДА СчетФактура.СуммаНДСДоставки * ВТ_КурсыДокОсн.КурсРегВал * ВТ_КурсыДокОсн.Кратность / (ВТ_КурсыДокОсн.Курс * ВТ_КурсыДокОсн.КратностьРегВал)
	|					ИНАЧЕ СчетФактура.СуммаНДСДоставки
	|				КОНЕЦ
	|		КОНЕЦ КАК ЧИСЛО(15, 2)),
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА СчетФактура.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА (СчетФактура.СтоимостьДоставкиБезНДС + СчетФактура.СуммаНДСДоставки) * ВТ_КурсыДокОсн.КурсРегВал * ВТ_КурсыДокОсн.Кратность / (ВТ_КурсыДокОсн.Курс * ВТ_КурсыДокОсн.КратностьРегВал)
	|			ИНАЧЕ СчетФактура.СтоимостьДоставкиБезНДС + СчетФактура.СуммаНДСДоставки
	|		КОНЕЦ КАК ЧИСЛО(15, 2)),
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА СчетФактура.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА СчетФактура.СтоимостьДоставкиБезНДС + СчетФактура.СуммаНДСДоставки
	|			ИНАЧЕ (СчетФактура.СтоимостьДоставкиБезНДС + СчетФактура.СуммаНДСДоставки) * ВТ_КурсыДокОсн.Курс * ВТ_КурсыДокОсн.КратностьРегВал / (ВТ_КурсыДокОсн.КурсРегВал * ВТ_КурсыДокОсн.Кратность)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)),
	|	ВЫРАЗИТЬ(ВЫБОР
	|			КОГДА СчетФактура.ВалютаДокумента = &ВалютаНациональная
	|				ТОГДА СчетФактура.СуммаНДСДоставки
	|			ИНАЧЕ СчетФактура.СуммаНДСДоставки * ВТ_КурсыДокОсн.Курс * ВТ_КурсыДокОсн.КратностьРегВал / (ВТ_КурсыДокОсн.КурсРегВал * ВТ_КурсыДокОсн.Кратность)
	|		КОНЕЦ КАК ЧИСЛО(15, 2)),
	|	СчетФактура.СтоимостьДоставкиБезНДС + СчетФактура.СуммаНДСДоставки,
	|	СчетФактура.СуммаНДСДоставки,
	...
	|	Документ.СчетФактура КАК СчетФактура
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТ_КурсыДокОсн КАК ВТ_КурсыДокОсн
	|		ПО (ВТ_КурсыДокОсн.ДокументОснование = СчетФактура.ДокументОснованиеДоставки),
	|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка,
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ЭтоУниверсальныйПередаточныйДокумент) Экспорт
	...
	|	Выбор когда СчетФактура.ДокументОснование ССЫЛКА Документ.АктВыполненныхРабот
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.СчетНаОплату
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.РасходнаяНакладная
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ОтчетОПереработке
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ПоступлениеВКассу
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ЗаказПокупателя
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.КорректировкаРеализации
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ПриемИПередачаВРемонт
	|		Тогда СчетФактура.ДокументОснование.ПодписьРуководителя.Должность
	...
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.КорректировкаРеализации
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ПриемИПередачаВРемонт
	|		Тогда СчетФактура.ДокументОснование.ПодписьРуководителя.РасшифровкаПодписи
	...
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.КорректировкаРеализации
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ПриемИПередачаВРемонт
	|		Тогда СчетФактура.ДокументОснование.ПодписьГлавногоБухгалтера.Должность
	...
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.КорректировкаРеализации
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ПриемИПередачаВРемонт
	|		Тогда СчетФактура.ДокументОснование.ПодписьГлавногоБухгалтера.РасшифровкаПодписи
	|		Иначе СчетФактура.ПодписьГлавногоБухгалтера.РасшифровкаПодписи Конец КАК РасшифровкаПодписиГлавногоБухгалтера,
	|	Выбор когда СчетФактура.ДокументОснование ССЫЛКА Документ.РасходнаяНакладная
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ОтчетОПереработке
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ЗаказПокупателя
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.КорректировкаРеализации
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ПриемИПередачаВРемонт
	|		Тогда СчетФактура.ДокументОснование.ПодписьКладовщика.Должность
	...
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.КорректировкаРеализации
	|				ИЛИ СчетФактура.ДокументОснование ССЫЛКА Документ.ПриемИПередачаВРемонт
	|		Тогда СчетФактура.ДокументОснование.ПодписьКладовщика.РасшифровкаПодписи
	...
	|	ВЫБОР КОГДА СчетФактура.ДокументОснование ССЫЛКА Документ.РасходнаяНакладная 
	|		ТОГДА СчетФактура.ДокументОснование.Вес 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК Вес,
	|	ВЫБОР КОГДА СчетФактура.ДокументОснование ССЫЛКА Документ.РасходнаяНакладная 
	|		ТОГДА СчетФактура.ДокументОснование.Объем 
	|		ИНАЧЕ 0 
	|	КОНЕЦ КАК Объем,
	...
	|	СчетФактура.НоменклатураДоставки,
	|	ВЫБОР
	|		КОГДА (ВЫРАЗИТЬ(СчетФактура.НоменклатураДоставки.НаименованиеПолное КАК СТРОКА(1000))) = """"
	|			ТОГДА СчетФактура.НоменклатураДоставки.Наименование
	|		ИНАЧЕ ВЫРАЗИТЬ(СчетФактура.НоменклатураДоставки.НаименованиеПолное КАК СТРОКА(1000))
	|	КОНЕЦ КАК ПредставлениеНоменклатурыДоставки,
	|	СчетФактура.НоменклатураДоставки.Код КАК КодНоменклатурыДоставки,
	|	СчетФактура.НоменклатураДоставки.Артикул КАК АртикулНоменклатурыДоставки,
	|	СчетФактура.НоменклатураДоставки.ЕдиницаИзмерения КАК ЕдиницаИзмеренияНоменклатурыДоставки,
	|	СчетФактура.НоменклатураДоставки.ЕдиницаИзмерения.Код КАК КодЕдиницыИзмеренияНоменклатурыДоставки,
	|	СчетФактура.СтоимостьДоставкиБезНДС,
	|	СчетФактура.СуммаНДСДоставки,
	|	СчетФактура.СтавкаНДСДоставки
	|ИЗ
	|	Документ.СчетФактура КАК СчетФактура
	...
	Пока Шапка.Следующий() Цикл
		...
		Если ЭтоСводныйСчетФактура Тогда
			...
			Для каждого Строка Из ТаблицаПокупателей Цикл
				...
				Строка.СведенияОГрузополучателе	= УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Грузополучатель, Шапка.ДатаДокумента,,);
		...
		Иначе
			...
			Грузоотправитель = ?(Шапка.ОнЖе, Шапка.Поставщик, Шапка.Грузоотправитель);
			СведенияОГрузоотправителе	= УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Грузоотправитель,Шапка.ДатаДокумента,,);
		...
		Запрос.УстановитьПараметр("ПечататьДоставку", ПечататьДоставку);
		...
		|	СчетФактура.НоменклатураДоставки,
		|	НЕОПРЕДЕЛЕНО,
		|	СчетФактура.НоменклатураДоставки.ЕдиницаИзмерения,
		|	1,
		|	СчетФактура.СтоимостьДоставкиБезНДС,
		|	СчетФактура.СтоимостьДоставкиБезНДС,
		|	СчетФактура.СтавкаНДСДоставки,
		...
		ИначеЕсли Шапка.ВидОперации = Перечисления.ВидыОперацийСчетФактура.Аванс 
			...
			|	ИЗ
			|		Документ.СчетФактура КАК СчетФактура
			|			ЛЕВОЕ СОЕДИНЕНИЕ Документ.СчетФактура.Запасы КАК СчетФактураЗапасы
			...
			|		СчетФактура.СтавкаНДСДоставки,
			|		СчетФактура.СуммаНДСДоставки,
			|		СчетФактура.СтоимостьДоставкиБезНДС + СчетФактура.СуммаНДСДоставки) КАК ВложенныйЗапрос
			|
			|УПОРЯДОЧИТЬ ПО
			|	ID,
			|	НомерСтроки";
		...
		Если ЭтоУниверсальныйПередаточныйДокумент Тогда
			...
			Если ЗначениеЗаполнено(Шапка.ДокументОснование) Тогда
				...
				МассаИОбъем = Новый Массив;
				Если ЗначениеЗаполнено(Шапка.Вес) Тогда
					МассаИОбъем.Добавить(
					СтрШаблон(НСтр("ru = 'масса брутто: %1'"), НРег(МассаГрузаПрописью(Шапка.Вес))));
				КонецЕсли; 
				Если ЗначениеЗаполнено(Шапка.Объем) Тогда
					МассаИОбъем.Добавить(
					СтрШаблон(НСтр("ru = 'объем: %1'"), ОбъемГрузаПрописью(Шапка.Объем)));
				КонецЕсли;
				ОбластьМакета.Параметры.Заполнить(Новый Структура("ДанныеОТранспортировке", СтрСоединить(МассаИОбъем, ", ")));
				
				Если (ТипЗнч(Шапка.ДокументОснование) = Тип("ДокументСсылка.РасходнаяНакладная") 
					ИЛИ ТипЗнч(Шапка.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации")
					ИЛИ ТипЗнч(Шапка.ДокументОснование) = Тип("ДокументСсылка.ОтчетОПереработке")
					ИЛИ ТипЗнч(Шапка.ДокументОснование) = Тип("ДокументСсылка.ЗаказПокупателя"))
				Тогда
					...
					Тогда
						...
						ОбластьМакета.Параметры.Основание = 
							Шапка.ОснованиеПечати + НСтр("ru ='; по доверенности №'") + Шапка.ДокументОснование.ДоверенностьНомер 
					...
					ИначеЕсли ПустаяСтрока(Шапка.ОснованиеПечати) Тогда
						
						Если ТипЗнч(Шапка.ДокументОснование) = Тип("ДокументСсылка.КорректировкаРеализации") Тогда
							
							ИсходныйДокумент = ПечатьДокументовУНФ.ПолучитьИсходныйДокументКорректировки(Шапка.ДокументОснование);
							ОбластьМакета.Параметры.Основание = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ИсходныйДокумент, "ОснованиеПечати").ОснованиеПечати;
					...
					Иначе
						...
						ОбластьМакета.Параметры.Основание = Шапка.ОснованиеПечати;
						
					КонецЕсли;
				
				ИначеЕсли ТипЗнч(Шапка.ДокументОснование) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
					
					Если ПустаяСтрока(Шапка.ОснованиеПечати) Тогда
...
Функция МассаГрузаПрописью(Знач ИтогоМассаБрутто)
...
Функция ОбъемГрузаПрописью(Знач ИтогоОбъем)
...
Функция ПечатнаяФормаКорректировочныхСчетовФактур(МассивОбъектов, ОбъектыПечати, ЭтоУниверсальныйКорректировочныйДокумент) Экспорт
	...
	Для Каждого ВыборкаСФ ИЗ ДанныеСчетовФактур Цикл
		...
		УправлениеПечатью.ЗадатьОбластьПечатиДокумента(ТабДокумент, НомерСтрокиНачало, ОбъектыПечати, ВыборкаСФ.СчетФактура);
