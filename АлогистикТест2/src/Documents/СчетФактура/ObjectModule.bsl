...
Процедура ЗаполнитьПодписиОтветственныхЛиц(ДанныеЗаполнения)
...
Процедура ОбработчикЗаполнения(ДанныеЗаполнения) Экспорт
	...
	ЗаполнитьПодписиОтветственныхЛиц(ДанныеЗаполнения);
...
Процедура ЗаполнитьТабличнуюЧастьЗапасы(СтрокаРасшифровкиПлатежа, ДокументЗапасов)
	...
	ЭтоЗаказПокупателя = (ТипЗнч(ДокументЗапасов) = Тип("ДокументСсылка.ЗаказПокупателя"));
	
	ВсегоПоДокументуЗапасов = 0;
	КоличествоСтрок = 0;
	Для каждого СтрокаТЧ Из ДокументЗапасов[ИмяТабличнойЧасти] Цикл
		
		Если ЭтоЗаказПокупателя И (СтрокаТЧ.НомерВариантаКП <> ДокументЗапасов.ОсновнойВариантКП ИЛИ СтрокаТЧ.ЭтоРазделитель) Тогда
			Продолжить;
		КонецЕсли;
	...
	Для каждого СтрокаДокумента Из ДокументЗапасов[ИмяТабличнойЧасти] Цикл
		...
		Если ЭтоЗаказПокупателя И (СтрокаДокумента.НомерВариантаКП <> ДокументЗапасов.ОсновнойВариантКП ИЛИ СтрокаДокумента.ЭтоРазделитель) Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьПоПрочимДокументам(ДанныеЗаполнения) Экспорт
	...
	Запрос = Новый Запрос(ТекстЗапросаПоПрочимДокументам(ДанныеЗаполнения));
	Запрос.УстановитьПараметр("Ссылка", ДанныеЗаполнения);
	...
	ЗаполнитьЗначенияСвойств(ЭтотОбъект, ВыборкаИзРезультатаЗапроса);
	ЭтотОбъект.СтавкаНДСДоставки = УправлениеНебольшойФирмойПовтИсп.ПолучитьСтавкуНДСРасчетная(ЭтотОбъект.СтавкаНДСДоставки);
	ЭтотОбъект.ДокументОснование = ДанныеЗаполнения;
	...
	ТаблицаЗапасов = ВыборкаИзРезультатаЗапроса[ИмяТЧЗапасы(ДанныеЗаполнения)].Выгрузить();
...
Функция ТекстЗапросаПоПрочимДокументам(ДанныеЗаполнения)
	...
	|	&ТЧПредоплата КАК ТЧПредоплата,
	|	&Грузоотправитель КАК Грузоотправитель,
	|	&ПараметрыДоставки КАК ПараметрыДоставки,
	|	Документ.Ссылка КАК ДокументОснование,
	|	Документ.Организация,
	|	Документ.Контрагент,
	|	Документ.Договор КАК Договор,
	...
	ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "&ВидОперации КАК ВидОперации,", ФрагментВидОперации(ДанныеЗаполнения));
	ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "&ТЧЗапасы КАК ТЧЗапасы,", ФрагментТЧЗапасы(ДанныеЗаполнения));
	ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "&ТЧРаботы КАК ТЧРаботы,", ФрагментТЧРаботы(ДанныеЗаполнения));
	ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "&ТЧПредоплата КАК ТЧПредоплата,", ФрагментТЧПредоплата(ДанныеЗаполнения));
	ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "&Грузоотправитель КАК Грузоотправитель,", ФрагментГрузоотправитель(ДанныеЗаполнения));
	ШаблонЗапроса = СтрЗаменить(ШаблонЗапроса, "&ПараметрыДоставки КАК ПараметрыДоставки,", ФрагментПараметрыДоставки(ДанныеЗаполнения));
	Результат =  СтрЗаменить(ШаблонЗапроса, "&Документ", ОбщегоНазначения.ИмяТаблицыПоСсылке(ДанныеЗаполнения));
	
	Возврат Результат;
	
КонецФункции

Функция ФрагментВидОперации(ДанныеЗаполнения)
	
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.СчетНаОплату") Тогда
		Возврат "ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.Аванс) КАК ВидОперации,";
	КонецЕсли;
	
	Если ТипЗнч(ДанныеЗаполнения) <> Тип("ДокументСсылка.КорректировкаРеализации") Тогда
		Возврат "ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.Продажа) КАК ВидОперации,";
	КонецЕсли;
	...
	Если ВидОперацииКорректировки = Перечисления.ВидыОперацийИсправленияПоступленияРеализации.СогласованноеИзменение Тогда
		Результат = "ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.Корректировка) КАК ВидОперации,";
	Иначе
		Результат = "ЗНАЧЕНИЕ(Перечисление.ВидыОперацийСчетФактура.Продажа) КАК ВидОперации,
		|	ИСТИНА КАК Исправление,";
	КонецЕсли;
	
	Возврат СтрШаблон("%1
	|	Документ.НомерИсправления,",
	Результат);
	
КонецФункции

Функция ФрагментТЧЗапасы(ДанныеЗаполнения)
	...
	|		Количество,
	|		ВЫБОР
	|			КОГДА Документ.%1.Количество = 0
	|				ТОГДА 0
	|			КОГДА Документ.СуммаВключаетНДС
	|				ТОГДА Выразить((Документ.%1.Сумма - Документ.%1.СуммаНДС) / Документ.%1.Количество КАК Число (15,2))
	|			ИНАЧЕ Документ.%1.Сумма / Документ.%1.Количество
	|		КОНЕЦ КАК Цена,
	|		ВЫБОР
	|			КОГДА Документ.СуммаВключаетНДС
	|				ТОГДА Документ.%1.Сумма / ((Документ.%1.СтавкаНДС.Ставка + 100) / 100)
	|			ИНАЧЕ Документ.%1.Сумма
	|		КОНЕЦ КАК Сумма,
	|		СтавкаНДС,
	|		СуммаНДС,%4
	|		Всего
	|	),",
	ИмяТЧЗапасы(ДанныеЗаполнения),
	ПартияТЧЗапасы(ДанныеЗаполнения),
	СодержаниеТЧЗапасы(ДанныеЗаполнения),
	СтранаНомерГТДТЧЗапасы(ДанныеЗаполнения));
	
КонецФункции

Функция ИмяТЧЗапасы(ДанныеЗаполнения)
	
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.АктВыполненныхРабот") Тогда
		Возврат "РаботыИУслуги";
	КонецЕсли;
	
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтчетОПереработке") Тогда
		Возврат "Продукция";
	КонецЕсли;
...
Функция ПартияТЧЗапасы(ДанныеЗаполнения)
...
Функция СодержаниеТЧЗапасы(ДанныеЗаполнения)
...
Функция СтранаНомерГТДТЧЗапасы(ДанныеЗаполнения)
...
Функция ФрагментТЧРаботы(ДанныеЗаполнения)
...
Функция ФрагментТЧПредоплата(ДанныеЗаполнения)
...
Функция ФрагментГрузоотправитель(ДанныеЗаполнения)
...
Функция ФрагментПараметрыДоставки(ДанныеЗаполнения)
	
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.РасходнаяНакладная")
		Или ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.СчетНаОплату") Тогда
		Возврат
		"		ВЫБОР
		|			КОГДА Документ.НоменклатураДоставки = ЗНАЧЕНИЕ(Справочник.Номенклатура.ПустаяСсылка)
		|				ТОГДА 0
		|			КОГДА Документ.СуммаВключаетНДС
		|				ТОГДА Документ.СтоимостьДоставки - Документ.СуммаНДСДоставки
		|			ИНАЧЕ Документ.СтоимостьДоставки
		|		КОНЕЦ КАК СтоимостьДоставкиБезНДС,
		|		Документ.СуммаНДСДоставки КАК СуммаНДСДоставки,
		|		Документ.СтавкаНДСДоставки КАК СтавкаНДСДоставки,
		|		Документ.НоменклатураДоставки КАК НоменклатураДоставки,
		|		Документ.Ссылка КАК ДокументОснованиеДоставки,";
	КонецЕсли;
	
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПокупателя")Тогда
		Возврат
		"		ВЫБОР
		|			КОГДА Документ.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.ПустаяСсылка)
		|				ИЛИ Документ.СпособДоставки = ЗНАЧЕНИЕ(Перечисление.СпособыДоставки.Самовывоз)
		|				ТОГДА 0
		|			КОГДА Документ.СуммаВключаетНДС
		|				ТОГДА Документ.СтоимостьДоставки - Документ.СуммаНДСДоставки
		|			ИНАЧЕ Документ.СтоимостьДоставки
		...
		|			ИНАЧЕ Документ.НоменклатураДоставки
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ВидОперации = Перечисления.ВидыОперацийСчетФактура.Аванс Тогда
		...
		Если ПолучитьФункциональнуюОпцию("УчетГТД") Тогда
			...
			СтраныЕАЭС = ГрузовыеТаможенныеДекларацииСервер.ТаблицаСтранТаможенногоСоюза().ВыгрузитьКолонку("Ссылка");
			
			Для каждого СтрокаТаблицыЗапасов Из Запасы Цикл
				
				Если ЗначениеЗаполнено(СтрокаТаблицыЗапасов.НомерГТД)
					И (НЕ ЗначениеЗаполнено(СтрокаТаблицыЗапасов.СтранаПроисхождения)
						ИЛИ СтрокаТаблицыЗапасов.СтранаПроисхождения = Справочники.СтраныМира.Россия)
					Тогда
				...
				Если НЕ ВидОперации = Перечисления.ВидыОперацийСчетФактура.НаАвансКомитента
					И НЕ ЗначениеЗаполнено(СтрокаТаблицыЗапасов.НомерГТД)
					И ЗначениеЗаполнено(СтрокаТаблицыЗапасов.СтранаПроисхождения)
					И СтраныЕАЭС.Найти(СтрокаТаблицыЗапасов.СтранаПроисхождения) = Неопределено
					Тогда
	...
	Если НеобходимоОтразитьРублевыеСуммыДокумента() Тогда
		Если ДокументыОснования.Количество() > 1 Тогда
			...
			Если СтоимостьДоставкиБезНДС > 0 И НЕ ЗначениеЗаполнено(ДокументОснованиеДоставки) Тогда
				ТекстОшибки = НСтр("ru = 'На закладке ""Доставка"" не указан документ-основание.'");
		...
		ИначеЕсли ДокументыОснования.Количество() = 1 И СтоимостьДоставкиБезНДС > 0 И НЕ ЗначениеЗаполнено(ДокументОснованиеДоставки) Тогда
			
			ТекстОшибки = НСтр("ru = 'На закладке ""Доставка"" не указан документ-основание.'");
			
			УправлениеНебольшойФирмойСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстОшибки, , , "ДокументОснованиеДоставки", Отказ);
			
		КонецЕсли;
	КонецЕсли;
	
	// Доставка
	Если СтоимостьДоставкиБезНДС>0 Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СуммаДокумента = Запасы.Итог("Всего") + СтоимостьДоставкиБезНДС + СуммаНДСДоставки;
	...
	Если НеобходимоОтразитьРублевыеСуммыДокумента() И ДокументыОснования.Количество() = 1 Тогда
		...
		Для Каждого ТекущаяСтрока Из Предоплата Цикл
			ТекущаяСтрока.ДокументОснование = ДокументыОснования[0].ДокументОснование;
		КонецЦикла;
		ДокументОснованиеДоставки = ДокументыОснования[0].ДокументОснование;
	КонецЕсли;
