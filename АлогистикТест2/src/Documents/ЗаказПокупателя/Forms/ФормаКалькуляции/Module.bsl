...
Процедура УстановитьУсловноеОформление()
	...
	Отбор.Использование 	= Истина;
	Отбор.ЛевоеЗначение 	= Новый ПолеКомпоновкиДанных("КалькуляцияНаФорме.Источник");
	Отбор.ПравоеЗначение 	= Перечисления.ИсточникиСтрокКалькуляции.Доставка;
...
Процедура ЗаполнитьПоШаблонуСервер()
	...
	Для каждого СтрокаСостава Из СоставШаблона Цикл
		НоваяСтрока = ДанныеЗаказа.Калькуляция.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, СтрокаСостава, "Номенклатура, Характеристика, Спецификация, Количество, ЕдиницаИзмерения");
...
Функция СравнитьКалькуляциюИСпецификацию(Идентификатор)
	...
	|		-(ВЫРАЗИТЬ(&КоличествоЗапасов / СпецификацииОперации.КоличествоПродукции КАК ЧИСЛО(15, 3))),
	|		ЛОЖЬ,
	|		ИСТИНА
	|	ИЗ
...
Процедура ВывестиКалькуляцию(Идентификатор = Неопределено)
	...
	Если Идентификатор=Неопределено Тогда
		...
		Запрос.УстановитьПараметр("ТаблицаЗапасов", ДанныеЗаказа.Запасы.Выгрузить(Новый Структура("ЭтоРазделитель", Ложь)));
	...
	// Доставка
	Если  Идентификатор=Неопределено И ЗначениеЗаполнено(ДанныеЗаказа.СлужбаДоставки) И ЗначениеЗаполнено(ДанныеЗаказа.СтоимостьДоставки) Тогда
		СтруктураОтбора = Новый Структура;
		СтруктураОтбора.Вставить("Источник", Перечисления.ИсточникиСтрокКалькуляции.Доставка);
		Строки = КалькуляцияНаФорме.НайтиСтроки(СтруктураОтбора);
		...
		КонецЕсли; 
		СтрокаТабличнойЧасти.Сумма = ДанныеЗаказа.СтоимостьДоставки;
		СтрокаТабличнойЧасти.Прибыль = СтрокаТабличнойЧасти.Сумма-СтрокаТабличнойЧасти.Себестоимость;
	КонецЕсли; 
...
Процедура ЗаполнитьТаблицуМатериалов(ТаблицаМатериалов, ИдентификаторСтрокиКалькуляции = Неопределено)
	...
	Для каждого Стр Из Строки Цикл
		Если ИдентификаторСтрокиКалькуляции = Неопределено И Стр.ЭтоРазделитель Тогда
			Продолжить;
		КонецЕсли;
...
Функция ТаблицаСоставаСпецификаций(СтрокиСпецификаций, СоставыСпецификаций, Уровень)
	...
	|	ЗНАЧЕНИЕ(Справочник.КлассификаторЕдиницИзмерения.ч),
	|	ЗНАЧЕНИЕ(Справочник.Спецификации.ПустаяСсылка),
	|	1 / СпецификацииОперации.КоличествоПродукции
	|ИЗ
	|	Справочник.Спецификации.Операции КАК СпецификацииОперации
	|ГДЕ
