...
Процедура КассаПриИзменении(Элемент)
	...
	Объект.ВалютаДенежныхСредств = ?(ЗначениеЗаполнено(Объект.ВалютаДенежныхСредств), Объект.ВалютаДенежныхСредств, СтруктураДанных.ВалютаПоУмолчанию);
	...
	Если ЗначениеЗаполнено(ДокументОбъект.ДокументОснование) 
		...
		Если ДокументОбъект.ВалютаДенежныхСредств = СтруктураРеквизитов.ВалютаДокумента
			...
			Если ТипЗнч(ДокументОбъект.ДокументОснование) = Тип("ДокументСсылка.ПриемИПередачаВРемонт") Тогда
				...
				СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(СтруктураДопРеквизитов.СтавкаНДС, Истина); 
...
Процедура ДополнитьАвансамиИОплатамиПоТаблицеДокумента(ДокументОбъект, ОбщиеПараметры)
	...
	Для каждого ТекСтрока Из ДокументОбъект.РасшифровкаПлатежа Цикл
		СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(ТекСтрока.СтавкаНДС, Истина); 
		СтрокаПозицииЧека = МенеджерОборудованияКлиентСервер.ПараметрыФискальнойСтрокиЧека(); 
...
Процедура ДополнитьТоварамиПоДокументу(ДокументОбъект, ОбщиеПараметры, ТабличнаяЧасть = "Запасы")
	...
	// Доставка
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ДокументОбъект)) И ДокументОбъект.Метаданные().Реквизиты.Найти("НоменклатураДоставки")<>Неопределено Тогда
		СтруктураРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДокументОбъект, "НоменклатураДоставки, СтоимостьДоставки, СтавкаНДСДоставки, СуммаНДСДоставки");
		Если ЗначениеЗаполнено(СтруктураРеквизитов.НоменклатураДоставки) И СтруктураРеквизитов.СтоимостьДоставки>0 Тогда
			СтавкаНДС = УправлениеНебольшойФирмойПовтИсп.ПолучитьЗначениеСтавкиНДС(СтруктураРеквизитов.СтавкаНДСДоставки, Истина);
			СтрокаПозицииЧека = МенеджерОборудованияКлиентСервер.ПараметрыФискальнойСтрокиЧека(); 
			СтрокаПозицииЧека.Наименование = Строка(СтруктураРеквизитов.НоменклатураДоставки);
			СтрокаПозицииЧека.Количество = 1;
			СтрокаПозицииЧека.Цена = СтруктураРеквизитов.СтоимостьДоставки + ?(ДокументОбъект.СуммаВключаетНДС, 0, СтруктураРеквизитов.СуммаНДСДоставки);
...
Процедура УстановитьВидимостьПечатиЧека()
	...
	Если ИспользоватьПО
		И Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеВКассу.ОтПокупателя Тогда
