...
Процедура УстановитьНадписьВИнформационнойПанели(Форма)
	...
	Если ЭтоАктСверкиРасчетов Тогда
		Элементы.ПоянениеОНазначенииСверки.Заголовок = 
			НСтр("ru = 'Акт сверки используется для сопоставления данных с налоговой инспекцией по налогам (пеням, штрафам) и позволяет выявить переплату или задолженность.'");
	...
	ИначеЕсли ЭтоСправкаОбИсполненииОбязанностейПоУплате Тогда
		Элементы.ПоянениеОНазначенииСверки.Заголовок = 
			НСтр("ru = 'Справка позволяет проверить наличие неисполненной обязанности по уплате налогов (пеней, штрафов). Часто требуется при участии в тендере, получении кредита, оформлении лицензии для подтверждения добросовестности налогоплательщика.'");
	КонецЕсли;
...
Процедура ОткрытьФормуВыбораНалогов(МножественныйВыбор, Обработчик, ТекущиеДанные = Неопределено, ЗапретитьВыборКБК = Ложь)
	...
	СтруктураПараметров.Вставить("ДатаНа", 				Объект.ДатаОкончанияПериода);
...
Процедура ПроверитьПередЗаписью(Отказ, ТаблицаКБК)
	...
	Если Объект.КоличествоНалогов = Перечисления.КоличествоНалоговДляСверкиИОН.Один Тогда
		...
		Иначе
			...
			Если НЕ ЗначениеЕстьВМакете(ТаблицаКБК, Налог) Тогда
	...
	ИначеЕсли Объект.КоличествоНалогов = Перечисления.КоличествоНалоговДляСверкиИОН.Несколько Тогда
		...
		Для каждого СтрокаТаблицыДокумента Из ТаблицаКБКиНалогов Цикл
			...
			Если НЕ СтроковыеФункцииКлиентСервер.ТолькоЦифрыВСтроке(СтрокаТаблицыДокумента.Значение) Тогда
				...
				Если НЕ ЗначениеЕстьВМакете(ТаблицаКБК, СтрокаТаблицыДокумента.Значение) Тогда
...
&НаСервере
Функция ЗначениеЕстьВМакете(ТаблицаКБК, Значение)
	
	Для каждого СтрокаТаблицыКБК Из ТаблицаКБК Цикл
		
		НазваниеПапкиВторогоУровня = Сред(СтрокаТаблицыКБК.Папка, 15);
		
		ЗначениеЕстьВТаблице = 
			Значение = СтрокаТаблицыКБК.Налог
			ИЛИ Значение = НСтр("ru = 'Прочие'")
			ИЛИ Значение = НазваниеПапкиВторогоУровня;
			
		Если ЗначениеЕстьВТаблице Тогда
			Возврат Истина;
		КонецЕсли;
...
Процедура СкопироватьТаблицуКБКПоНалогуВОбъект(Отказ, ТекущийОбъект)
	...
	Иначе
		...
		ТаблицаКБК = Документы.ЗапросНаИнформационноеОбслуживаниеНалогоплательщика.ТаблицаКБКСФильтрамиПоСвойствамОрганизации(
			ТекущийОбъект.Организация, 
			, 
			ТекущийОбъект.ДатаОкончанияПериода);
		...
		|		КОГДА ПОДСТРОКА(ТаблицаКБК.Папка, 4, 6) = ""Прочие""
		|				И ТаблицаДокумента.Значение = ""Прочие""
		|			ТОГДА ""Прочие""
		|		КОГДА ПОДСТРОКА(ТаблицаКБК.Папка, 15, 200) = ТаблицаДокумента.Значение
		|			ТОГДА ПОДСТРОКА(ТаблицаКБК.Папка, 15, 200)
		|		ИНАЧЕ """"
		|	КОНЕЦ КАК Налог,
		|	ТаблицаДокумента.ОКАТО
		|ИЗ
		...
		|				ИЛИ ПОДСТРОКА(ТаблицаКБК.Папка, 4, 6) = ""Прочие""
		|					И ТаблицаДокумента.Значение = ""Прочие""
		|				ИЛИ ПОДСТРОКА(ТаблицаКБК.Папка, 15, 200) = ТаблицаДокумента.Значение)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ТаблицаДокумента.НомерСтроки";
...
Процедура ЗадатьПравилаОтображенияДатыНаДляАктов(Форма)
	...
	Если ЭтоАктСверкиРасчетов Тогда
		Если НачалоДня(Объект.ДатаОкончанияПериода) = НачалоДня(ТекущаяДата()) Тогда
...
Процедура ЗадатьЗначениеПолучателяПоУмолчанию()
	...
	Иначе
		...
		Если ВсеФНСДляДаннойОрганизации.Количество() = 1 Тогда
			...
			ПолучательОдин = Истина;
			ПолучательПредставление = Строка(ПолучательПоУмолчанию);
