...
Процедура Оплатить(Команда)
	...
	Если НЕ ПроверкаЗаполненияПрошлаУспешно() Тогда
		Возврат;
	КонецЕсли;
	
	Если Объект.Запасы.Итог("Всего") <= 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОплатитьОбработкаОтветаНаВопрос", ЭтотОбъект, СуммаДоРасчетаАвтоСкидок),
	...
	Иначе
		ОплатитьФрагмент(СуммаДоРасчетаАвтоСкидок);
	КонецЕсли;
	
КонецПроцедуры

&НаСервере
Функция ПроверкаЗаполненияПрошлаУспешно()
...
Процедура ОплатитьОбработкаОтветаНаВопрос(РезультатОтвета, ДополнительныеПараметры) Экспорт
	...
	Если РезультатОтвета = КодВозвратаДиалога.Да Тогда
		ОплатитьФрагмент(ДополнительныеПараметры);
	Иначе
		Возврат;
	КонецЕсли;
...
//
&НаКлиенте
Процедура ОплатитьФрагмент(СуммаДоРасчетаАвтоСкидок)
	...
	СтруктураПараметров.Вставить("ОплатаПлатежнымиКартами", Объект.ОплатаПлатежнымиКартами);
...
Процедура ОплатитьЗавершение(Результат, Параметры) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		ИначеЕсли Элементы.СтраницыДанныеПоСтрокеИСдача.ТекущаяСтраница = Элементы.СтраницаСообщений Тогда
