...
Процедура ЗаполнитьНазначениеПлатежа()
	...
	Если ПустаяСтрока(ТекстПлатежа)
		...
		И (ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.СчетНаОплатуПоставщика") ИЛИ
		ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.РасходДСПлан")
		И ЗначениеЗаполнено(ДокументОснование.ДокументОснование))
		Тогда
		
		ТекНомер = "";
		Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.СчетНаОплатуПоставщика")
			И ЗначениеЗаполнено(ДокументОснование.НомерВходящегоДокумента) Тогда
			ТекНомер = ДокументОснование.НомерВходящегоДокумента;
			ТекДата = ДокументОснование.ДатаВходящегоДокумента;
		ИначеЕсли ДокументОснование.ДокументОснование.Метаданные().Реквизиты.Найти("НомерВходящегоДокумента") <> Неопределено
			И ЗначениеЗаполнено(ДокументОснование.ДокументОснование.НомерВходящегоДокумента) Тогда
...
Процедура ЗаполнитьПоНачислениеНалогов(ДокументОснование) Экспорт
	...
	Если НЕ РезультатЗапроса.Пустой() Тогда
		...
		ИначеЕсли Выборка.ВидНалога = Справочники.ВидыНалогов.ПФРНакопительная
			...
			Если РегламентированнаяОтчетностьПереопределяемый.ЭтоЮридическоеЛицо(ЭтотОбъект.Организация) Тогда
				СтатусСоставителя =   "01";
			Иначе
				СтатусСоставителя =   "09";
			КонецЕсли;
