...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.ИспользоватьОблачнуюТелефонию = ПолучитьФункциональнуюОпцию("ИспользоватьОблачнуюТелефонию");
	ЭтотОбъект.ИспользоватьМобильнуюТелефонию = ПолучитьФункциональнуюОпцию("ИспользоватьМобильнуюТелефонию");
	ЭтотОбъект.ПерсональноеМобильноеУстройство = ТелефонияСервер.ПерсональноеМобильноеУстройство();
	Элементы.НабратьНомер.Видимость = (ИспользоватьМобильнуюТелефонию И ЗначениеЗаполнено(ПерсональноеМобильноеУстройство))
		Или ИспользоватьОблачнуюТелефонию;
	Элементы.КонтактыИндексПиктограммы.Видимость = Элементы.НабратьНомер.Видимость;
	Если Не Элементы.НабратьНомер.Видимость Тогда
		Элементы.КонтрагентКакСвязаться.МаксимальнаяШирина = 35;
	КонецЕсли;
	Элементы.ПрослушатьЗапись.Видимость = ЗначениеЗаполнено(ИдентификаторЗаписиРазговора);
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "ОбщаяФорма.ВыборЗначенияКонтактнойИнформации" И ЗначениеЗаполнено(ВыбранноеЗначение) Тогда
		...
		ИначеЕсли ИспользоватьОблачнуюТелефонию Тогда
			НабратьНомерОблачнаяТелефонияСервер(ВыбранноеЗначение, Объект.Ссылка);
		КонецЕсли;
	КонецЕсли;
...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	ЭтотОбъект.ИдентификаторЗаписиРазговора = ТелефонияСервер.ПолучитьИдентификаторЗаписиРазговора(ТекущийОбъект.Ссылка);
...
Процедура ПрослушатьЗапись(Команда)
	...
	ШаблонURL = ТелефонияКлиентСервер.КорневойАдресОблачногоПровайдера() + "queries/recording/issa/[recording_id]/[action]";
	
	ПараметрыURL = Новый Структура;
	ПараметрыURL.Вставить("recording_id", ИдентификаторЗаписиРазговора);
	ПараметрыURL.Вставить("action", "play");
	
	ПерейтиПоНавигационнойСсылке(СтроковыеФункцииКлиентСервер.ВставитьПараметрыВСтроку(ШаблонURL, ПараметрыURL));
...
&НаСервереБезКонтекста
Процедура НабратьНомерОблачнаяТелефонияСервер(КонтрагентКакСвязаться, Событие)
	
	ТелефонияСервер.НабратьНомерОблачнаяТелефония(КонтрагентКакСвязаться, Событие.УникальныйИдентификатор());
...
Процедура НабратьНомерКлиент(Номер)
	...
	ИначеЕсли ИспользоватьОблачнуюТелефонию Тогда
		НабратьНомерОблачнаяТелефонияСервер(Номер, Объект.Ссылка);
	КонецЕсли;
...
Процедура ПоказатьОповещениеОНабореНомера(Номер)
	...
	ИначеЕсли ИспользоватьОблачнуюТелефонию Тогда
		...
		ПоказатьОповещениеПользователя(
			НСтр("ru='Набор номера'"),
			,
			СтрШаблон(ШаблонПояснения, Номер),
			БиблиотекаКартинок.ТелефонныйЗвонокИсходящий
		);
