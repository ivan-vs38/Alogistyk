...
Функция ТекстЗапросаУниверсальный()
	...
	|	,КлючСвязи
	|	,Номенклатура
	|	,Выбор когда (ВЫРАЗИТЬ(Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000))) = """" Тогда Запасы.Номенклатура.Наименование
...
Процедура СформироватьИнвентаризациюЗапасов(ОписаниеПечатнойФормы, МассивОбъектов, ОбъектыПечати)
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Пока ВыборкаСтрокТовары.Следующий() Цикл
			...
			ПараметрыНоменклатуры.Вставить("ПредставлениеАртикула", ВыборкаСтрокТовары.Артикул);
			ПараметрыНоменклатуры.Вставить("ПредставлениеАртикула", ВыборкаСтрокТовары.Артикул);
			ПараметрыНоменклатуры.Вставить("СтрокаСерийныеНомера", РаботаССерийнымиНомерами.СтрокаСерийныеНомераИзВыборки(ВыборкаСерийныеНомера, ВыборкаСтрокТовары.КлючСвязи));
