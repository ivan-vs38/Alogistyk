...
Процедура ИнициализироватьФорму()
	...
	// Заполним реквизиты для управления видимостью страниц.
	//НеЗаполненПараметрСумма = НЕ ЗначениеЗаполнено(Параметры.Сумма);
	//НеВерныйФорматСуммы = Параметры.НеВерныйФорматСуммы;
	//ЕстьНесколькоДокументов = Параметры.ЕстьНесколькоДокументов;
	//ЕстьОписаниеОшибок = Параметры.Свойство("ОписаниеОшибок");
	// Конец заполнения.
	...
	//Элементы.ТаблицаНайденныхДокументов.РастягиватьПоВертикали = Параметры.ЕстьНесколькоДокументов;
	...
	Если ВидОперацииВходящие = ВидОперацииПрочееПоступление ИЛИ
		ВидОперацииИсходящие = ВидОперацииПрочееСписание Тогда
...
Процедура УправлениеВидимостьюСтраницДляПеремещенияДенег()
	...
	//Элементы.ДействияСоСпискомНайденныхДокументов.Видимость = ЕстьНесколькоДокументов;
	//Элементы.СтраницаОписаниеОшибок.Видимость = ЕстьОписаниеОшибок;
	//Элементы.ДействияССуммой.Видимость = НеВерныйФорматСуммы ИЛИ НеЗаполненПараметрСумма;
...
Процедура ЗаполнитьИнформациюОДоговореНаСервере(Контрагент, Договор, ПараметрыДоговора)
	...
	Иначе
		ДоговорНаименование = ДоговорНаименованиеПраваяЧасть;
		// Если контрагент уже создан и у него есть договор, то новый договор будем предлагать создать с номером и датой, а не с наименованием "Основной договор".
...
Процедура СформироватьИнформационныеНадписи()
	...
	//Элементы.ИнформацияОРасчетномСчете.Заголовок = ""; //"Расчетный счет также не будет создан";
	//Элементы.Группа3.ЦветФона = ЦветаСтиля.ЦветФонаФормы;
...
Процедура УстановитьВидимостьИДоступностьЭлементов()
	...
	//Элементы.ГруппаСуществующего1.ТолькоПросмотр = ПустаяСтрока(ИспользоватьСуществущего);
	//Элементы.ГруппаНового1.ТолькоПросмотр = ПустаяСтрока(СоздаватьНового);
	...
	// Контрагент.
	Если НЕ ПустаяСтрока(СоздаватьНового) Тогда
		//Элементы.КомандаОК.Заголовок = НСтр("ru='Записать и выбрать'");
	...
	ИначеЕсли НЕ ПустаяСтрока(СоздаватьНовогоПриЗагрузкеВыпискиВ1С) Или НЕ ПустаяСтрока(НеСоздавать) Тогда
		//Элементы.КомандаОК.Заголовок = НСтр("ru='ОК'");
		Элементы.КонтрагентСсылка.АвтоОтметкаНезаполненного = Ложь;
		Элементы.КонтрагентСсылка.ОтметкаНезаполненного = Ложь;
	ИначеЕсли НЕ ПустаяСтрока(ИспользоватьСуществующего) Тогда
		//Элементы.КомандаОК.Заголовок = НСтр("ru='Выбрать'");
...
Функция ОбработатьОКНаСервере()
	...
	//	Если (ВидОперацииВходящие = Перечисления.ВидыОперацийПоступлениеНаСчет.ВзносНаличными
	//		ИЛИ ВидОперацииИсходящие = Перечисления.ВидыОперацийРасходСоСчета.СнятиеНаличных)
	//		И КассаОрганизации.Пустая() Тогда
	//		
	//		ТекстСообщения = НСтр("ru = 'Необходимо выбрать кассу организации.'");
	//		ЕстьОшибкиЗаполнения = Истина;
	//		Поле = "КассаОрганизации";
	//		
	//	ИначеЕсли (ВидОперацииВходящие = Перечисления.ВидыОперацийПоступлениеНаСчет.ПереводСДругогоСчета
	//		ИЛИ ВидОперацииИсходящие = Перечисления.ВидыОперацийРасходСоСчета.ПереводНаДругойСчет)
	//		И СчетОрганизации.Пустая() Тогда
	//		
	//		ТекстСообщения = НСтр("ru = 'Необходимо выбрать счет организации.'");
	//		ЕстьОшибкиЗаполнения = Истина;
	//		Поле = "СчетОрганизации";
	//		
	//	КонецЕсли;
	//	
	...
	ИначеЕсли Не ПустаяСтрока(ИспользоватьСуществующего) Тогда
		...
		Если Не ПустаяСтрока(СоздаватьНовыйСчетКонтрагента) Или Не ПустаяСтрока(СоздаватьНовыйДоговор) Тогда
			...
			Иначе
				ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru='Вначале укажите контрагента на закладке ""Контрагент""'"));
				Возврат Ложь;
			КонецЕсли;
...
Процедура КонтрагентСсылкаПриИзмененииНаСервере()
	...
	// Возможна следующая ситуация. Был найден счет контрагента и подставлен контрагент. В базе есть несколько контрагентов с одинаковым счетом (бюджетные организации).
	// В этом случае при изменении контрагента нужно обновлять информацию о расчетном счете!!!
...
Функция ЗаполнитьНаименованиеДоговора()
	...
	Иначе
		//ДоговорНаименование = ?(ПустаяСтрока(ДоговорНаименование), "", ДоговорНаименование + " ") + ДоговорНаименованиеПраваяЧасть;
	КонецЕсли;
...
Процедура ВидОперацииИсходящиеПриИзмененииНаСервере()
	...
	Элементы.Корреспонденция.Видимость = (ВидОперацииИсходящие = ВидОперацииПрочееСписание);
...
Процедура ЗаполнитьИННиКППИзВыписки(Команда)
	...
	Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) ИЛИ
		...
		Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) Тогда
			СписокВыбора.Добавить("ИНН", НСтр("ru = 'Обновить ИНН из виписки '")+"("+КонтрагентИНН+")");
		Иначе
		...
		// ИНН + КПП.
		Если СокрЛП(КонтрагентИНН) <> СокрЛП(Элементы.КонтрагентСсылкаИНН.ТекстРедактирования) И
			СокрЛП(КонтрагентКПП) <> СокрЛП(Элементы.КонтрагентСсылкаКПП.ТекстРедактирования) Тогда
...
Процедура ЗаполинтьСпискиВыбораВидовОпераций()
	...
	Элементы.ВидОперацииВходящие.СписокВыбора.ЗагрузитьЗначения(СписокВидовОпераций.ВыгрузитьЗначения());

	СписокВидовОпераций = ДвиженияДенежныхСредствВызовСервера.ПолучитьСписокВидовОперацийРасходДСБанк();
	Элементы.ВидОперацииИсходящие.СписокВыбора.ЗагрузитьЗначения(СписокВидовОпераций.ВыгрузитьЗначения());
