	...
	Для Ш = 1 по 3 Цикл 
		...
		Если Область <> Неопределено Тогда 
			...
			Для Нс = НомерСтроки по МакетСтиля.ВысотаТаблицы Цикл
				...
				Попытка
					Объект[Свойство] = Вычислить(Значение);
				Исключение
				КонецПопытки;
...
Процедура ПолучитьКонтекстЭДОРезультат(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Свойство("КонтекстЭДО") Тогда 
		...
		Если ДополнительныеПараметры.УчетнаяЗаписьДокументооборота <> Неопределено Тогда 
			...
			ЗапрашиватьПодтверждениеОПрименении = Ложь; //1.1.8
			...
			Попытка //1.1.8
				КонтекстЭДОКлиент.АвтоматическаяНастройкаУчетнойЗаписи(
					...
					ВыполняемоеОповещение, ЗапрашиватьПодтверждениеОПрименении, 
...
Процедура ВсеПроверкиИСборСведенийЗавершены() Экспорт
	...
	ИмяФормыПодробности = "%1.Форма.ДиагностикаОтчетности_ПодробностиИРекомендации";
	ИмяФормыПодробности = СтрЗаменить(ИмяФормыПодробности, "%1", ТипОбъекта);
...
Процедура ВыполнитьПоискВНайденныхСертификатах(Результат, ВхПараметры) Экспорт
	...
	Иначе
		...
		КриптографияЭДКОСлужебныйКлиент.ПолучитьСертификаты(ОповещениеОЗавершении, Хранилище, Ложь, РезультатыПодключенияКК.Криптокомпонент, Ложь);
...
Процедура ДекорацияТекстОбщийНажатие(Ссылка, ВхПараметры) Экспорт 
	...
	Если РазделНайден Тогда 
		...
		ПараметрыОткрытия.Вставить("Организация", ОрганизацияКлиент);
		ПараметрыОткрытия.Вставить("УчетнаяЗаписьДокументооборота", УчетнаяЗаписьОбменаКлиент);
		ПараметрыОткрытия.Вставить("Автонастройка", РезультатыДиагностики.Автонастройка);
		ПараметрыОткрытия.Вставить("ТелефонСлужбыПоддержки", ТелефонСлужбыПоддержки);
		...
		Форма = ПолучитьФорму(ИмяФормыПодробности); //, ПараметрыОткрытия, ЭтаФорма
		Форма.ВладелецФормы = ЭтаФорма;
		ЗаполнитьЗначенияСвойств(Форма.Параметры, ПараметрыОткрытия);
		Форма.Открыть();
...
Функция ИПИспользуетТрудНаемныхРаботников(Организация) Экспорт
	...
	Иначе
		...
		Попытка
			Выполнить("РегламентированнаяОтчетностьПереопределяемый.ПолучитьПризнакИспользованияИПТрудаНаемныхРаботников(Организация, ИПИспользуетТрудНаемныхРаботников)"); //BRO119
		Исключение
			Выполнить("ИПИспользуетТрудНаемныхРаботников = РегламентированнаяОтчетностьПереопределяемый.ИПИспользуетТрудНаемныхРаботников(Организация)"); //BRO118
		КонецПопытки;
