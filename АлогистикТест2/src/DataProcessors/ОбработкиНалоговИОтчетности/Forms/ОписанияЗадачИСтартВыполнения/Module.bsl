...
Процедура Подключаемый_ПроверитьВыполнениеДлительнойОперацииНаКлиенте()
	...
	Если СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "РасчетПоСтраховымВзносам") > 0 Тогда
		...
		Исключение
			...
			ВызватьИсключение;
		КонецПопытки;
	ИначеЕсли СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "Декларация12") > 0 Тогда
		ОперацияЗавершена = РегламентированнаяОтчетностьАЛКОКлиент.ВыполнитьДействияПриПроверкеВыполненияДлительнойОперации(ФормаРегламентированногоОтчета);
	КонецЕсли;
	
	Если ОперацияЗавершена Тогда
		Если СтрНайти(СтруктураВспомДанныхДлитОперации.ИмяФормыРезультата, "РасчетПоСтраховымВзносам") > 0 Тогда
			ФормаРегламентированногоОтчета.ЗагрузитьПодготовленныеДанные();
		КонецЕсли;
...
Процедура ОбработатьПараметрСобытияКалендаря()
	...
	ИначеЕсли СобытиеКалендаряДанные.Задача = Справочники.ЗадачиКалендаряПодготовкиОтчетности.РасчетПоСтраховымВзносам Тогда
		...
		Элементы.ВыполнитьЗадачу.Заголовок = НСтр("ru='Сформировать отчет по страховым взносам »'");
	КонецЕсли;
...
Процедура ВыполнитьРасчетНажатие()
	...
	Если Результат.Свойство("ИмяФормыРезультата") Тогда
		...
		ИначеЕсли СтрНайти(ИмяФормыРезультата, "Декларация12") > 0 Тогда
			...
			СтруктураВспомДанныхДлитОперации = Новый Структура;
			СтруктураВспомДанныхДлитОперации.Вставить("ВидДлительнойОперации", ВидДлительнойОперации);
			СтруктураВспомДанныхДлитОперации.Вставить("ВидПечати", Неопределено);
			СтруктураВспомДанныхДлитОперации.Вставить("ФормаРегламентированногоОтчета", ФормаОтчета);
			СтруктураВспомДанныхДлитОперации.Вставить("ИмяФормыРезультата", ИмяФормыРезультата);
			
			
			ФормаОтчета.ВыполнитьДлительнуюОперацию_НаКлиенте(ВидДлительнойОперации,, ЭтаФорма);
