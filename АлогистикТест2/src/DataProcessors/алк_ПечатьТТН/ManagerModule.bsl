#Если Сервер Или ТолстыйКлиентОбычноеПриложение Или ВнешнееСоединение Тогда
	...
	//
	Процедура ПолучитьДанныеПоТранспортировке(ДанныеДокумента, ТекущийДокумент)
		
		Запрос = Новый Запрос;
		Запрос.УстановитьПараметр("Документ",  ТекущийДокумент);
		Запрос.Текст = 
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		|	алк_ТранспортировкаЗапасов.Номер,
		|	алк_ТранспортировкаЗапасов.Ссылка КАК ТекущийДокумент,
		|	алк_ТранспортировкаЗапасов.Дата КАК ДатаДокумента,
		|	алк_ТранспортировкаЗапасов.Организация,
		|	алк_ТранспортировкаЗапасов.Организация.Префикс КАК Префикс,
		|	алк_ТранспортировкаЗапасов.Организация КАК ЮрФизЛицо,
		|	алк_ТранспортировкаЗапасов.Организация КАК Поставщик,
		|	алк_ТранспортировкаЗапасов.СтруктурнаяЕдиницаОтправитель КАК СтруктурнаяЕдиница,
		|	алк_ТранспортировкаЗапасов.Запасы.(
		|		НомерСтроки,
		|		Номенклатура,
		|		ВЫБОР
		|			КОГДА (ВЫРАЗИТЬ(алк_ТранспортировкаЗапасов.Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000))) = """"
		|				ТОГДА алк_ТранспортировкаЗапасов.Запасы.Номенклатура.Наименование
		|			ИНАЧЕ ВЫРАЗИТЬ(алк_ТранспортировкаЗапасов.Запасы.Номенклатура.НаименованиеПолное КАК СТРОКА(1000))
		|		КОНЕЦ КАК ЗапасНаименование,
		|		Номенклатура.Код КАК КодПродукции,
		|		Номенклатура.Артикул КАК Артикул,
		|		Характеристика,
		|		"""" КАК Содержание,
		|		КоличествоПакетов КАК Количество,
		|		0 КАК КоличествоМест
		|	),
		|	алк_ТранспортировкаЗапасов.Покупатель,
		|	алк_ТранспортировкаЗапасов.Грузополучатель,
		|	алк_ТранспортировкаЗапасов.Грузоотправитель,
		|	"""" КАК АдресДоставки,
		|	"""" КАК МаркаАвтомобиля,
		|	алк_ТранспортировкаЗапасов.Перевозчик,
		|	алк_ТранспортировкаЗапасов.ИдентификаторТС КАК ГосНомерАвтомобиля
		|ИЗ
		...
		//ДоставкаСервер.ДобавитьСтрокуДоставкиТТН(ВыборкаДанныхДокумента, ТабличнаяЧастьДокумента);
		ДанныеДокумента.Вставить("ТабличнаяЧастьДокумента", ТабличнаяЧастьДокумента);
		
	КонецПроцедуры // ПолучитьДанныеПоТранспортировке()
	...
	Процедура ПолучитьДанныеПоРасходнойНакладной(ДанныеДокумента, ТекущийДокумент)
		...
		|	РасходнаяНакладная.НоменклатураДоставки.ЕдиницаИзмерения КАК ЕдиницаИзмеренияНоменклатурыДоставки,
		|	РасходнаяНакладная.НоменклатураДоставки.ЕдиницаИзмерения.Код КАК КодЕдиницыИзмеренияНоменклатурыДоставки,
	...
	Процедура ПолучитьДанныеПоКорректировкеРеализации(ДанныеДокумента, ТекущийДокумент)
		...
		|	КорректировкаРеализации.НоменклатураДоставки.ЕдиницаИзмерения КАК ЕдиницаИзмеренияНоменклатурыДоставки,
		|	КорректировкаРеализации.НоменклатураДоставки.ЕдиницаИзмерения.Код КАК КодЕдиницыИзмеренияНоменклатурыДоставки,
	...
	Процедура ПолучитьДанныеПоОтчетуОПереработке(ДанныеДокумента, ТекущийДокумент)
		...
		ТабличнаяЧастьОтходы = ВыборкаДанныхДокумента.Отходы.Выгрузить();
	...
	Функция МассаГрузаПрописью(Знач ИтогоМассаБрутто, Знач ДанныеОбъектаВес)
		...
		"ДП=Истина",
		"тонн,тонн,тонн,ж,килограмм,килограмма,килограммов,м,3");
	...
	Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
		...
		Для каждого ТекущийДокумент Из МассивОбъектов Цикл
			...
			Если ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.алк_ТранспортировкаЗапасов") Тогда
				
				ПолучитьДанныеПоТранспортировке(ДанныеДокумента, ТекущийДокумент);
			...
			СведенияОПоставщике       = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.ЮрФизЛицо,        Шапка.ДатаДокумента, , Неопределено);	
			...
			СведенияОГрузоотправителе = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Грузоотправитель, Шапка.ДатаДокумента, , ?(Шапка.Грузоотправитель = Шапка.ЮрФизЛицо, Неопределено, Неопределено));
			...
			СведенияОПокупателе       = УправлениеНебольшойФирмойСервер.СведенияОЮрФизЛице(Шапка.Покупатель,       Шапка.ДатаДокумента, , Неопределено);
			...
			Если ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.РасходнаяНакладная")
				...
				Для каждого ВыборкаСтрокЗапасы Из ТабличнаяЧастьДокумента Цикл
					...
					ОбластьМакета.Параметры.Масса = МассаБрутто;
			...
			ИначеЕсли ТипЗнч(ТекущийДокумент) = Тип("ДокументСсылка.алк_ТранспортировкаЗапасов")  Тогда
				...
				Для каждого ВыборкаСтрокЗапасы Из ТабличнаяЧастьДокумента Цикл
					...
					Иначе
						//ОбластьМакета.Параметры.ЗапасНаименование = УправлениеНебольшойФирмойСервер.ПолучитьПредставлениеНоменклатурыДляПечати(ВыборкаСтрокЗапасы.ЗапасНаименование, 
						//ВыборкаСтрокЗапасы.Характеристика, ВыборкаСтрокЗапасы.Артикул);
						ОбластьМакета.Параметры.ЗапасНаименование = ВыборкаСтрокЗапасы.Характеристика.Наименование; 
					КонецЕсли;
			...
			//КонецЕсли; 
			//Если ИтогоМассаБрутто>0 ИЛИ Шапка.Вес>0 Тогда
			//	ОтпущеноНаСуммуПрописью = ОтпущеноНаСуммуПрописью+?(ПустаяСтрока(ОтпущеноНаСуммуПрописью), "", ", ")+
			//	НСтр("ru = 'масса брутто: '")+НРег(МассаГрузаПрописью(ИтогоМассаБрутто, Шапка.Вес));
			//КонецЕсли; 
			//Если ИтогоОбъем>0 ИЛИ Шапка.Объем>0  Тогда
			//	ОтпущеноНаСуммуПрописью = ОтпущеноНаСуммуПрописью+?(ПустаяСтрока(ОтпущеноНаСуммуПрописью), "", ", ")+
			...
			Если ИтогоМассаБрутто >= 1 Тогда
				ОбластьМакета.Параметры.МассаГрузаБрутто = Окр(ИтогоМассаБрутто * КоэффициентПересчетаВТонны, 3, РежимОкругления.Окр15как20);
				ОбластьМакета.Параметры.МассаГрузаБруттоПрописью = МассаГрузаПрописью(ИтогоМассаБрутто, Шапка.Вес);
			КонецЕсли;
	...
	Процедура ОткрытьФормуЗаполненияДанных() Экспорт
