...
Функция СформироватьОписаниеПравилОтправкиУНФ()
	...
	ТекстОписания = НСтр("ru = 'Правила отправки данных из программы ""Управление нашей фирмой"" :'") + Символы.ПС;
	...
	Иначе
		...
		Если ИспользоватьОтборПоОрганизациямУНФ Тогда
			...
			ПредставлениеКоллекции = ПланыОбмена.ОбменУправлениеНебольшойФирмойБухгалтерия30.СокращенноеПредставлениеКоллекцииЗначений(КоллекцияЗначений);
	...
	Если ПереноситьАктыКакРеализациюУслуг Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Документы ""Акт выполненных работ"" переносятся в ""Бухгалтерию предприятия"" как документы ""Реализация товаров и услуг""'");
		ТекстОписания = СтрЗаменить(ТекстОписания, "%ПредставлениеКоллекции%", ПредставлениеКоллекции);
	КонецЕсли;
...
Функция СтруктураСоответствияНастройкиОтборовРеквизитамФормы()
	...
	СтруктураНастроек.Вставить("ПереноситьАктыКакРеализациюУслуг", "ПереноситьАктыКакРеализациюУслуг");
