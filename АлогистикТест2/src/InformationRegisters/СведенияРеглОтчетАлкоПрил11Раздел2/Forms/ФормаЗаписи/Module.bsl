...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если (НЕ ПустаяСтрока(СтрокаРегистрСведенийЛицензииПоставщиковАлкогольнойПродукции)) 
		...
		ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			...
			|ЛицензииПоставщиковАлкогольнойПродукции.Период КАК ДатаВыдачи,
			|ЛицензииПоставщиковАлкогольнойПродукции.ДатаОкончания КАК ДатаОкончания,
			|ЛицензииПоставщиковАлкогольнойПродукции.СерияНомер КАК СерияНомер,
		...
		ДинСписокПоставщика.ОсновнаяТаблица = СтрокаРегистрСведенийЛицензииПоставщиковАлкогольнойПродукции;
...
Процедура УправлениеВидимостью(ПоказатьВыборПроизводителей = Ложь, ПоказатьВыборПоставщиков = Ложь)
	...
	ИначеЕсли ПоказатьВыборПоставщиков Тогда
		...
		Если НЕ Запись.Поставщик = Неопределено Тогда
			...
			Если ДинСписокПоставщика.ОсновнаяТаблица = СтрокаРегистрСведенийЛицензииПоставщиковАлкогольнойПродукции Тогда
			
				// Для регистра нужно определить ключ записи для указания текущей строки,
				// но для этого нужно определить вид лицензии.
				// Серию-номер не задаем, поскольку возможны разные варианты заполнения этого поля.
				ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
		               |	ЛицензииПоставщиковАлкогольнойПродукции.ВидЛицензии КАК ВидЛицензии,
		               ...
		               |	ЛицензииПоставщиковАлкогольнойПродукции.ВидЛицензии <> ЗНАЧЕНИЕ("
					   + СтрокаПеречислениеВидыЛицензийНаПродажуАлкоголяПиво + ")
		               |	И ЛицензииПоставщиковАлкогольнойПродукции.Поставщик = &Поставщик";
					   
					   
				ЗапросВидаЛицензии = Новый Запрос;
				ЗапросВидаЛицензии.Текст = ТекстЗапроса;
				
				ЗапросВидаЛицензии.Параметры.Вставить("Поставщик", Запись.Поставщик);
				...
				Если КолСтрокРезультата > 0 Тогда
					...
					ДатаНачала =  Результат[0].ДатаНачала;
					...
					Если КолСтрокРезультата > 1 Тогда
						...
						Для Инд = 0 По КолСтрокРезультата - 1 Цикл
							...
							ТекДатаНачала 		= ТекСтрокаРезультата.ДатаНачала;
							ТекДатаОкончания 	= ТекСтрокаРезультата.ДатаОкончания;
							ТекКемВыдана 		= ТекСтрокаРезультата.КемВыдана;
							...
							Если (СтрНайти(ТекСерияНомер, СокрЛП(Запись.П000020000391)) > 0)
								...
								и (ТекДатаОкончания = Запись.П000020000311)
								и (ТекКемВыдана = СокрЛП(Запись.П000020000312))	Тогда
				...
				Если (НЕ ВидЛицензии = Неопределено) и (НЕ ДатаНачала = Неопределено) Тогда
					...
					ИмяРегистраСведенийЛицензииПоставщиковАлкогольнойПродукции = 
						СтрЗаменить(СтрокаРегистрСведенийЛицензииПоставщиковАлкогольнойПродукции, "РегистрСведений.", "");
