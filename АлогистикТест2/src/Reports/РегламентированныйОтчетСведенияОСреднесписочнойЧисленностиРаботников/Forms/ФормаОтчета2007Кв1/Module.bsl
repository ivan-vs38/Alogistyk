...
Процедура ЗаполнитьПоказателиОтчетаНаСервере(ПоказателиОтчета)
	...
	ЭтаФорма["СтруктураДанныхТитульный"].ДатаПериод = ДатаНаКоторуюУказанаСреднесписочнаяЧисленность(ЭтаФорма["СтруктураДанныхТитульный"].ДатаПериод);
...
Функция ДатаНаКоторуюУказанаСреднесписочнаяЧисленность(ДатаПериод)
	...
	Возврат Дата(Лев(Прав(СокрЛП(ДатаПериод), 7), 4) + Формат(НомерМесяца(СтруктураРеквизитовФормы, ДатаПериод), "ЧЦ=2; ЧВН=") + "01");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если ПустаяСтрока(Титульный.ДатаПериод) Тогда
		...
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтаФорма, "Не указана дата, по состоянию на которую указывается среднесписочная численность!", "Титульный", "ДатаПериод");
		СтатусПроверки = Ложь;
...
Процедура ЗаполнитьАвтоНаСервере()
	...
	РегламентированнаяОтчетностьПереопределяемый.ЗаполнитьОтчет(Сред(Лев(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") - 1), 7),
																Сред(ЭтаФорма.ИмяФормы, СтрНайти(ЭтаФорма.ИмяФормы, ".Форма.") + 7),
