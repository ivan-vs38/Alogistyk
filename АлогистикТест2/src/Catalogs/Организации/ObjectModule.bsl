...
Процедура ПриЗаписи(Отказ)
	...
	ЗаписатьНастройкиФаксимилеПоУмолчанию();
...
Процедура ЗаписатьНастройкиФаксимилеПоУмолчанию()
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Организация", Ссылка);
	Запрос.Текст = 
	"Выбрать Первые 1 *
	|Из РегистрСведений.ФаксимилеНастройки КАК РСНастройкиФаксимиле
	|Где РСНастройкиФаксимиле.Организация = &Организация";
	
	РезультатЗапроса = Запрос.Выполнить();
	Если РезультатЗапроса.Пустой() Тогда
		
		ОписаниеФорм = ПодписьДокументовУНФ.ШаблонТаблицыФаксимильныхПодписей();
		ПодписьДокументовУНФ.ПечатныеФормыПоддерживающиеФаксимильнуюПодпись(ОписаниеФорм, Ссылка);
		
		НаборЗаписей = РегистрыСведений.ФаксимилеНастройки.СоздатьНаборЗаписей();
		НаборЗаписей.Отбор.Организация.Установить(Ссылка, Истина);
