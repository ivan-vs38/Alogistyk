...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьКоличествоВСписке = ?(ОтборОстатки=0 И НЕ ПоказыватьОстатки, Ложь, Истина);
	
	ПолучитьТекстЗапросаСписокЗапасы();
...
&НаСервере
Функция ПолучитьТекстЗапросаСписокЗапасы()
	...
	Если ПоказыватьЦены И ИспользоватьКоличествоВСписке Тогда
		СписокЗапасы.ТекстЗапроса =	СписокЗапасы.ТекстЗапроса +"
		|	,ВЫБОР
	...
	ИначеЕсли ИспользоватьКоличествоВСписке Тогда
		СписокЗапасы.ТекстЗапроса =	СписокЗапасы.ТекстЗапроса +"
	...
	Если ИспользоватьКоличествоВСписке Тогда
		СписокЗапасы.ТекстЗапроса =	СписокЗапасы.ТекстЗапроса +"
	...
	|	КОНЕЦ КАК ВариантКартинки,";
	Если ПоказыватьЦены Тогда
		СписокЗапасыВНаличии.ТекстЗапроса =	СписокЗапасыВНаличии.ТекстЗапроса +"
		|	ВЫБОР
		|		КОГДА ЦеныНоменклатурыСрезПоследних.ЕдиницаИзмерения ССЫЛКА Справочник.ЕдиницыИзмерения
	...
	Если ПоказыватьЦены Тогда
		СписокЗапасыВНаличии.ТекстЗапроса =	СписокЗапасыВНаличии.ТекстЗапроса +"
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ЦеныНоменклатуры.СрезПоследних(&Период, Актуальность И ВидЦен = &ВидЦен) КАК ЦеныНоменклатурыСрезПоследних
		|		ПО (ЦеныНоменклатурыСрезПоследних.Номенклатура = ЗапасыНаСкладах.Номенклатура.Ссылка)
